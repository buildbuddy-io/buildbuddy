...
MIN_DOCKER_VERSION = version.parse("19.03")
...
def check_buildx_support(stdout=sys.stdout, stderr=sys.stderr):
    ...
    match = re.fullmatch(
            r"Docker version ([0-9]+(?:[.][0-9]+){0,2})[^,]*, build \S+",
    ...
    if match != None:
        ...
        if kernel_version < MIN_KERNEL_VERSION:
            print(f"kernel version {kernel_version} does not have binfmt_misc fix-binary (F) support; need at least kernel version {MIN_KERNEL_VERSION}.", file=stderr)
            return False
    else:
    ...
    if match != None:
        ...
        if binfmts_version < MIN_BINFMTS_VERSION:
            print(f"update-binfmts version {binfmts_version} does not have fix-binary (F) support; need at least update-binfmts {MIN_BINFMTS_VERSION}.", file=stderr)
            return False
    else:
    ...
    with open(f"/proc/sys/fs/binfmt_misc/qemu-{canary_arch}") as f:
        ...
        if match != None:
            if "F" not in match.group(1):
                print(f"QEMU is not registered in binfmt_misc with fix-binary (F) flag. Flags: {match.group(1)}", file=stderr)
                return False
        else:
