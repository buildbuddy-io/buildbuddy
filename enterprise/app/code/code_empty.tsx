...
export default class CodeEmptyStateComponent extends React.Component {
  ...
  componentDidMount() {
    const request = new invocation.GetInvocationStatRequest({ aggregationType: invocation.AggType.REPO_URL_AGGREGATION_TYPE });
    rpcService.service
  ...
  render() {
    return <div className="">
      ...
      <div className="repo-previews">
        ...
        <div className="repo-previews-section">
          {RECOMMENDED_REPOS.map(this.renderRepo)}
        </div>
      </div>
    </div>
  }
  ...
  renderRepo(repo: string) {
    return <div className="repo-preview">
      <a href={`/code/${repo}`}>
        <img src={`https://opengraph.githubassets.com/678d0aac73c1525b882f63e6a2978a53b80d99d1788ddb16863183a38a66f98a/${repo}`} />
      </a>
    </div>
  }
