  ...
  private getSortValue(timeline: execution_stats.ExecutionTimeline, column: string): number | string {
    switch (column) {
      ...
      case DURATION_P50_COLUMN:
        return this.getP50(timeline.summary?.durationUsec);
      case CPU_P50_COLUMN:
        return this.getP50(timeline.summary?.cpuNanos);
      case MEMORY_P50_COLUMN:
        return this.getP50(timeline.summary?.peakMemory);
      case "shard":
            ...
            <tbody>
              {timelines.map((timeline, i) => (
                <tr key={i} className="target-data-table-row" onClick={() => this.handleRowClick(timeline)}>
                  ...
                  <td>{format.durationUsec(this.getP50(timeline.summary?.durationUsec))}</td>
                  <td>{format.durationMillis(this.getP50(timeline.summary?.cpuNanos) / 1e6)}</td>
                  <td>{format.bytes(this.getP50(timeline.summary?.peakMemory))}</td>
                </tr>
              ))}
            </tbody>
          </table>
