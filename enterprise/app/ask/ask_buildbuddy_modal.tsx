...
interface Props {
  ...
  invocationIds: string[];
  onRequestClose: () => void;
  onIncludeRepository: (invocationIds: string[]) => Promise<AskBuildBuddyRepository | undefined>;
...
export default class AskBuildBuddyModal extends React.Component<Props, State> {
  ...
  private async includeRepository() {
    this.setState({ repositoryLoading: true });
    try {
      const repository = await this.props.onIncludeRepository(this.props.invocationIds);
      if (repository) {
  ...
  render() {
    ...
    return (
      <Modal
        ...
        onAfterOpen={this.focusQuestion.bind(this)}
                ...
                <div className="ask-buildbuddy-context-items">
                  ...
                  {this.state.repository ? (
                    <button
                      ...
                      Repository {this.state.repository.repoUrl} <span aria-hidden="true">×</span>
                    </button>
                  ) : (
                    <button
                      className="ask-buildbuddy-include-context"
                      disabled={this.state.repositoryLoading}
                      onClick={this.includeRepository.bind(this)}>
