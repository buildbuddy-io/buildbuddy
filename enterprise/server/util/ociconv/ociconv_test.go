...
func TestOciconv(t *testing.T) {
	for _, useOCIFetcher := range []bool{false, true} {
		t.Run(fmt.Sprintf("useOCIFetcher=%v", useOCIFetcher), func(t *testing.T) {
			...
			path, err := ociconv.CreateDiskImage(ctx, resolver, root, ref, oci.Credentials{}, useOCIFetcher)
			require.NoError(t, err)
...
func TestOciconv_ChecksCredentials(t *testing.T) {
	for _, useOCIFetcher := range []bool{false, true} {
		t.Run(fmt.Sprintf("useOCIFetcher=%v", useOCIFetcher), func(t *testing.T) {
			...
			_, err = ociconv.CreateDiskImage(ctx, resolver, root, ref, oci.Credentials{}, useOCIFetcher)
			require.Error(t, err)
