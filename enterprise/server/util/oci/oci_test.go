			...
			for _, writeManifests := range []bool{false, true} {
				for _, readLayers := range []bool{false, true} {
					for _, writeLayers := range []bool{false, true} {
						name := tc.name + fmt.Sprintf("/read_manifests_%t_write_manifests_%t_read_layers_%t_write_layers_%t", readManifests, writeManifests, readLayers, writeLayers)
						t.Run(name, func(t *testing.T) {
							...
							layerGets := 1
							if readLayers && writeLayers {
								layerGets = 0
							}
							resolveAndCheck(tc.args.imageName+"_image", resolveGets, layerGets)
							...
							resolveAndCheck(tc.args.imageName+"_index", resolveGets, layerGets)
						})
					}
				}
			}
