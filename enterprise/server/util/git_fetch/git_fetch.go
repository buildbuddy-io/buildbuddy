...
const (
	...
	// maxGitTraceFileDescriptor is the largest descriptor supported by Git's
	// numeric trace targets.
	maxGitTraceFileDescriptor = 9
)
...
type Result struct {
	...
	Err error
	// Retries is the number of additional fetch attempts that were started.
	Retries int
...
// CommandRunner executes Git after applying configure to the exec.Cmd it will
// start. It must return an error without starting the command if configure
// fails.
type CommandRunner func(ctx context.Context, out io.Writer, configure func(*exec.Cmd) error, args ...string) (string, error)
...
func runAttempt(ctx context.Context, opts runOptions, slowFetchDetectionEnabled bool, command CommandRunner, args ...string) (*Result, error) {
	...
	trace2Processor, err := git_trace2.StartProcessor(processor.observeTrace2Event)
	if err != nil {
	...
	if slowFetchDetectionEnabled && trace2Processor != nil {
	...
	configure := func(cmd *exec.Cmd) error {
		if trace2Processor != nil {
			if err := trace2Processor.Attach(cmd); err != nil {
				return fmt.Errorf("attach git trace2 processor: %w", err)
			}
		}
		if packTraceWriter != nil {
			// os/exec maps ExtraFiles entries to consecutive descriptors starting
			// at 3. Trace2 and any caller-provided files occupy earlier entries.
			fd := 3 + len(cmd.ExtraFiles)
			if fd > maxGitTraceFileDescriptor {
				return fmt.Errorf("descriptor %d exceeds git's single-digit pack trace target limit", fd)
			}
			cmd.ExtraFiles = append(cmd.ExtraFiles, packTraceWriter)
			cmd.Env = append(cmd.Environ(), "GIT_TRACE_PACKFILE="+strconv.Itoa(fd))
		}
		return nil
	}

	fetchStart := time.Now()
	output, fetchErr := command(attemptCtx, opts.Output, configure, args...)
...
func (p *traceProcessor) observeTrace2Event(event *git_trace2.Event) {
	if event.Type == "data" && event.Category == "progress" && event.Key == "total_bytes" {
		n, err := strconv.ParseInt(event.StringValue(), 10, 64)
		if err == nil && n >= 0 {
			p.mu.Lock()
