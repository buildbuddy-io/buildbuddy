...
func (p *PebbleCache) newWrappedWriter(ctx context.Context, fileRecord *sgpb.FileRecord, key filestore.PebbleKey, shouldCompress bool, fileType sgpb.FileMetadata_FileType) (interfaces.CommittedWriteCloser, error) {
	...
	if shouldCompress {
		o := metrics.CompressionRatio.With(prometheus.Labels{metrics.CompressionType: "zstd", metrics.CacheNameLabel: p.name})
		return compression.NewZstdCompressingWriter(wc, p.bufferPool, fileRecord.GetDigest().GetSizeBytes(), o)
	}
	return wc, nil
}
