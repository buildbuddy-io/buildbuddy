...
func Register(env environment.Env) error {
	...
	if *forceCompaction {
		...
		start := time.Now()
		err := c.db.Compact([]byte{keys.MinByte}, []byte{keys.MaxByte}, true /*=parallelize*/)
...
func keyRange(key []byte) ([]byte, []byte) {
	return keyPrefix(key, []byte{keys.MinByte}), keyPrefix(key, []byte{keys.MaxByte})
}
...
func (p *PebbleCache) Statusz(ctx context.Context) string {
	...
	diskEstimateBytes, err := db.EstimateDiskUsage([]byte{keys.MinByte}, []byte{keys.MaxByte})
	if err == nil {
