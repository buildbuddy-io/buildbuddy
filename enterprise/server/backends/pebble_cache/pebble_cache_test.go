...
func TestSupportsEncryption(t *testing.T) {
	...
	// encryption.
	ctx := getAnonContext(t, te)
	supportsEncryption, err := pc.SupportsEncryption(ctx)
	require.NoError(t, err)
	require.False(t, supportsEncryption)
	...
	// Second user requests a partition override that does not have encryption
	// support.
	ctx = te.GetAuthenticator().AuthContextFromAPIKey(context.Background(), apiKey2)
	ctx = setMetadataOnIncomingContext(ctx, cacheutil.PartitionOverrideKey, pebble_cache.DefaultPartitionID)
	supportsEncryption, err = pc.SupportsEncryption(ctx)
