...
func TestBackfill(t *testing.T) {
	...
	for _, r := range resourcesWritten {
		if err := memoryCache3.DeleteDeprecated(ctx, r.GetDigest()); err != nil {
			t.Fatal(err)
		}
	}
...
func TestDelete(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		if err := distributedCaches[i%3].DeleteDeprecated(ctx, d); err != nil {
			t.Fatal(err)
		}
		for _, baseCache := range baseCaches {
			exists, err := baseCache.Contains(ctx, rn)
...
func TestDelete_NonExistentFile(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		err := distributedCaches[i%3].DeleteDeprecated(ctx, d)
		assert.NoError(t, err)
	}
}
