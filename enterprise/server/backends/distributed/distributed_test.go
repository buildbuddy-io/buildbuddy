...
func TestSupportsCompressor(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "if one peer does not support the compressor, should return false",
			cache1:   &testcompression.CompressionCache{Cache: env.GetCache()},
			cache2:   newMemoryCache(t, singleCacheSizeBytes),
			expected: false,
		},
	...
	for _, tc := range testCases {
		{
			...
			sc := dc1.SupportsCompressor(ctx, repb.Compressor_ZSTD, rn)
			require.Equal(t, tc.expected, sc)

			sc = dc2.SupportsCompressor(ctx, repb.Compressor_ZSTD, rn)
			require.Equal(t, tc.expected, sc)
		}
	}
}
