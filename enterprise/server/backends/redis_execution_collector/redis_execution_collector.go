...
func (c *collector) DeleteInProgressExecution(ctx context.Context, executionID string) error {
	// Proactively delete reverse invocation links for this execution. These are
	// only used to map back to the execution events, which are now being
	// deleted.
	serializedLinks, err := c.rdb.SMembers(ctx, getInvocationLinkKey(executionID)).Result()
	if err != nil && err != redis.Nil {
		log.CtxWarningf(ctx, "Failed to get invocation links for execution %q: %s", executionID, err)
	}
	links, err := unmarshalStoredInvocationLinks(serializedLinks)
	if err != nil {
		log.CtxWarningf(ctx, "Failed to unmarshal invocation links for execution %q: %s", executionID, err)
	}
	pipe := c.rdb.Pipeline()
	pipe.Del(ctx, getExecutionUpdatesKey(executionID)).Err()
	for i := range serializedLinks {
		pipe.LRem(ctx, getReverseInvocationLinkKey(links[i].GetInvocationId()), 1, serializedLinks[i])
	}
	_, err = pipe.Exec(ctx)
	return err
}
