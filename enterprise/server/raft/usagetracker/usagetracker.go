...
func (pu *partitionUsage) generateSamplesForEviction(ctx context.Context) error {
	...
	return pu.generateSamplesFromIndex(ctx, db)
}

// generateSamplesFromIndex feeds the eviction pipeline from the node-local
// atime index: entries sort by (partition, atime), so scanning forward from
// the partition's index start visits this node's records coldest-first, and a
// sweep can stop as soon as it reaches entries younger than minEvictionAge
// (nothing older can follow). Each entry is verified against the stored record
// before it is offered: an entry whose record is gone or whose atime doesn't
// match is an orphan (blind Set overwrites, cleared or removed ranges) and is
// deleted in place.
func (pu *partitionUsage) generateSamplesFromIndex(ctx context.Context, db pebble.IPebbleDB) error {
	...
	for {
		...
		// A fresh iterator per sweep: a sweep only touches the cold front of
		// the index, and a new snapshot per pass observes the deletions made
		// by the evictions the previous sweep fed.
		...
		for valid := iter.First(); valid; valid = iter.Next() {
			...
			if pu.clock.Since(time.UnixMicro(atimeUsec)) < pu.minEvictionAge {
				// Entries are atime-ordered: everything from here on is younger.
				break
			}
			// Verify the entry against the stored record; drop orphans (record
			// gone, or its atime moved) in place. The index is node-local
			// derived state, so the delete doesn't go through raft.
			fileMetadata.ResetVT()
			err = pebble.GetProto(db, fileKey, fileMetadata)
			if err != nil && !status.IsNotFoundError(err) {
				log.Warningf("cannot generate sample for eviction, skipping %q: %s", fileKey, err)
				continue
			}
			...
			pu.maybeAddToSampleChan(ctx, fileKey, fileMetadata, timer)
			offered++
		}
		iter.Close()

		if offered == 0 {
			// Nothing cold enough to evict yet; wait before rescanning.
			if !pu.samplerSleep(ctx) {
				return nil
			}
		}
...
func (pu *partitionUsage) maybeAddToSampleChan(ctx context.Context, fileKey []byte, fileMetadata *sgpb.FileMetadata, timer clockwork.Timer) {
	...
	keyBytes := make([]byte, len(fileKey))
	copy(keyBytes, fileKey)
