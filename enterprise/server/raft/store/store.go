...
func (s *Store) StartCluster(ctx context.Context, req *rfpb.StartClusterRequest) (*rfpb.StartClusterResponse, error) {
	log.Warningf("VVV start cluster")
	...
	if req.GetCloneRangeId() != 0 {
		r, err := s.GetReplica(req.GetCloneRangeId())
		if err != nil {
			return nil, err
		}
		destDir := r.DBDirForCluster(req.GetClusterId(), req.GetNodeId())
		if err := r.CloneDB(destDir); err != nil {
			return nil, err
		}
	}
...
func (s *Store) SplitCluster(ctx context.Context, req *rfpb.SplitClusterRequest) (*rfpb.SplitClusterResponse, error) {
	...
	bootStrapInfo.CloneRangeID = sourceRange.GetRangeId()
