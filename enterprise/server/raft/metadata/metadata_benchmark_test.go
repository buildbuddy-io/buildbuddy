...
// records owned by user1's group, and returns a leader Server, a context authenticated
...
func setUpBenchmarkCluster(b *testing.B) (*metadata.Server, context.Context, []*sgpb.FileRecord) {
	...
	*log.IncludeShortFileName = true
	...
	for range benchPoolSize {
		// "GR1" is user1's group in userMap (v7 keys require GR-prefixed IDs).
		md := randomFileMetadata(b, 100, "GR1")
