...
func (sm *Replica) ApplySnapshotFromReader(r io.Reader, db ReplicaWriter) error {
	...
	if err := wb.DeleteRange(keys.Key{keys.MinByte}, keys.Key{keys.MaxByte}, nil /*ignored write options*/); err != nil {
		return err
	}
...
func (sm *Replica) SaveSnapshot(preparedSnap interface{}, w io.Writer, quit <-chan struct{}) error {
	...
	defer snap.Close()
	return sm.SaveSnapshotToWriter(w, snap, []byte{keys.MinByte}, []byte{keys.MaxByte})
}
