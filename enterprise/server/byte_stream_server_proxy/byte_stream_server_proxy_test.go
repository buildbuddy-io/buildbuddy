...
func TestWriteCompressibleData_UncompressedToCompressed(t *testing.T) {
	compressibleData, err := os.ReadFile("./testdata/compressible_file.a")
	require.NoError(t, err)
	...
	flagMap := map[string]memprovider.InMemoryFlag{
		"cache_proxy.unconditionally_compress_writes": {
		...
		"cache_proxy.min_size_to_unconditionally_compress": {
			State:          memprovider.Enabled,
			DefaultVariant: "default",
			Variants: map[string]any{
				"default": int(1024),
	...
	provider := memprovider.NewInMemoryProvider(flagMap)
	openfeature.SetProviderAndWait(provider)
	fp, err := experiments.NewFlagProvider("test-app")
	require.NoError(t, err)
...
func TestWrite_SkipsCompression(t *testing.T) {
	rawData, err := os.ReadFile("./testdata/compressible_file.a")
	require.NoError(t, err)
