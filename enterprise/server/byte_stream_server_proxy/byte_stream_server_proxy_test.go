...
func runBSServer(ctx context.Context, env *testenv.TestEnv, t *testing.T) (bspb.ByteStreamClient, *atomic.Int32) {
...
func runBSProxy(ctx context.Context, client bspb.ByteStreamClient, localEnv *testenv.TestEnv, t *testing.T) bspb.ByteStreamClient {
	localEnv.SetByteStreamClient(client)
	byteStreamServer, err := New(localEnv)
	require.NoError(t, err)
	grpcServer, runFunc := testenv.RegisterLocalGRPCServer(localEnv)
	bspb.RegisterByteStreamServer(grpcServer, byteStreamServer)
	go runFunc()
	conn, err := testenv.LocalGRPCConn(ctx, localEnv)
	require.NoError(t, err)
	t.Cleanup(func() { conn.Close() })
...
func TestRead(t *testing.T) {
	...
	bs, requestCounter := runBSServer(ctx, remoteEnv, t)
