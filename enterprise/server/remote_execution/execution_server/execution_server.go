...
func (s *ExecutionServer) insertInvocationLinkInRedis(ctx context.Context, executionID, invocationID string, linkType sipb.StoredInvocationLink_Type) error {
	...
	// Only store the reverse link (invocationID => []executionID) if we're also
	// writing execution progress state to redis, since these links are only
	// used to list the in-progress state by invocation ID.
	storeReverseLink := *writeExecutionProgressStateToRedis
	return s.env.GetExecutionCollector().AddInvocationLink(ctx, link, storeReverseLink)
}
