...
func (r *commandRunner) pullCredentials() (credentials.Credentials, error) {
	return credentials.FromProperties(r.PlatformProperties)
}
...
func (p *pool) warmupImage(ctx context.Context, cfg *WarmupConfig) error {
	...
	creds, err := credentials.FromProperties(platProps)
	if err != nil {
		return err
	}
	err = container.PullImageIfNecessary(
