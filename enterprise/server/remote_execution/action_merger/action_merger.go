...
// reasonably quickly.
func MaybeMergeExecutions(ctx context.Context, rdb redis.UniversalClient, schedulerService interfaces.SchedulerService, adResource *digest.ResourceName, doNotCache bool) (string, Op) {
	newExecutionID, err := adResource.UploadString()
	if err != nil {
		// adResource is always a CAS resource name, so UploadString can't
		// return an error.
		panic(fmt.Sprintf("UploadString unexpectedly failed on %v: %v", adResource, err))
	}
