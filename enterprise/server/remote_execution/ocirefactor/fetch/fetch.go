...
// CacheFetcher fetches OCI artifacts from BuildBuddy's Action Cache and CAS.
...
// NewCacheFetcher creates a new CacheFetcher.
func NewCacheFetcher(acClient repb.ActionCacheClient, bsClient bspb.ByteStreamClient, secret string) *CacheFetcher {
	return &CacheFetcher{
		acClient: acClient,
		bsClient: bsClient,
		secret:   secret,
...
func (f *CacheFetcher) FetchManifest(ctx context.Context, ref string, platform *repb.Platform, creds *rgpb.Credentials) ([]byte, error) {
...
func (f *CacheFetcher) FetchBlobMetadata(ctx context.Context, ref string, creds *rgpb.Credentials) (int64, string, error) {
	digestRef, err := parseDigestRef(ref)
	if err != nil {
		return 0, "", err
	}
...
func (f *CacheFetcher) FetchBlob(ctx context.Context, ref string, creds *rgpb.Credentials) (io.ReadCloser, error) {
