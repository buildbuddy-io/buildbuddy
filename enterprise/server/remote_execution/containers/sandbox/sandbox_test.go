...
func TestSandboxedHelloWorld(t *testing.T) {
	...
	result := sandboxContainer.Run(ctx, cmd, tempDir, credentials.Credentials{})
...
func TestCrossContainerReads(t *testing.T) {
	...
	sandboxContainer := sandbox.New(&sandbox.Options{})
	goodResult := sandboxContainer.Run(ctx, goodCmd, tempDir1, credentials.Credentials{})
	evilResult := sandboxContainer.Run(ctx, evilCmd, tempDir2, credentials.Credentials{})
