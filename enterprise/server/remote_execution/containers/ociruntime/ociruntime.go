...
var (
	...
	overprovisionCPUs = flag.Int("executor.oci.overprovision_cpus", -1, "Additional CPU quota added to the task size, expressed as CPU count. The value -1 means no limit (except for the cpu_limit config option).")
...
func (c *ociContainer) createSpec(ctx context.Context, cmd *repb.Command) (*specs.Spec, error) {
	...
	if *overprovisionCPUs >= 0 {
		milliCPUHardLimit = c.milliCPU + int64(*overprovisionCPUs)
	}
	if *cpuLimit != 0 {
	...
	if milliCPUHardLimit > 0 {
		log.Debugf("CPU hard-limit: %dm", milliCPUHardLimit)
