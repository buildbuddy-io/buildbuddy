...
// Exec, then Remove.
func (c *FirecrackerContainer) Run(ctx context.Context, task *repb.ExecutionTask, actionWorkingDir string, fsLayout *container.FileSystemLayout) *interfaces.CommandResult {
	start := time.Now()
	defer func() {
