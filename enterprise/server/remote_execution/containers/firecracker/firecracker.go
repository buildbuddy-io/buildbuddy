...
func (c *FirecrackerContainer) LoadSnapshot(ctx context.Context) error {
	...
	maxFallbackAgeStr := platform.FindEffectiveValue(c.task, platform.MaxLocalFallbackSnapshotAgePropertyName)
	var maxFallbackAge *time.Duration
	...
	snap, err := c.loader.GetSnapshot(ctx, c.snapshotKeySet, &snaploader.GetSnapshotOptions{
		RemoteReadEnabled:           c.supportsRemoteSnapshots,
		ReadPolicy:                  readPolicy,
		MaxLocalFallbackSnapshotAge: maxFallbackAge,
	})
	if err != nil {
