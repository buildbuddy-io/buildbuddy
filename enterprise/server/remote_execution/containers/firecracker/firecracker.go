...
func (c *FirecrackerContainer) Exec(ctx context.Context, cmd *repb.Command, stdio *interfaces.Stdio) *interfaces.CommandResult {
	log.CtxInfof(ctx, "Executing command. From branch 1. Adding another change to branch 1.")
	...
	defer func() {
		...
		log.CtxDebugf(ctx, "Exec took %s (This was edited in a patch) changes in b1", execDuration)
