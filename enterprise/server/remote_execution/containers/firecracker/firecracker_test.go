...
func TestFirecrackerBalloon_Bazel_On(t *testing.T) {
	ctx := context.Background()
	...
	execAndPause := func(enableBalloon bool) int64 {
		...
		env := getTestEnv(ctx, t, envOpts{cacheSize: 25_000_000_000})
		env.SetAuthenticator(testauth.NewTestAuthenticator(testauth.TestUsers("US1", "GR1")))
		rootDir := testfs.MakeTempDir(t)
		workDir := testfs.MakeDirAll(t, rootDir, "work")
...
cd buildbuddy
git checkout a3048b94ead7cc966fde43ad636b9526127233ef
...
bazelisk build //server/util/status:status
	...
	bytesCachedWithBalloon := execAndPause(true)
	log.Warningf("Cached %v MB", bytesCachedWithBalloon/(1024*1024))
}
