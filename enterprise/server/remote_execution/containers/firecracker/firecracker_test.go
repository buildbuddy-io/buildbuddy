...
// Tests for NetworkMode/NetworkingEnabled logic.
// No Firecracker binary or privileged resources required.

// TestNetworkMode_Off confirms network=off without init-dockerd yields
// NETWORK_MODE_OFF (no network interface).
func TestNetworkMode_Off(t *testing.T) {
...
// TestNetworkingEnabled confirms NETWORK_MODE_OFF is the only mode where
// NetworkingEnabled returns false.
