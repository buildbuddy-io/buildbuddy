...
func TestFirecrackerRunWithDockerMirror(t *testing.T) {
	...
	cmd := &repb.Command{
		Arguments: []string{"bash", "-c", fmt.Sprintf(`
   		set -e
...
EOF
		kill -HUP $(cat /var/run/docker.pid)  # Reload docker config
   		...
   		docker pull %s/busybox &> /dev/null
   		docker run --rm %s/busybox echo Hello
   		docker run --rm %s/busybox echo world

   		docker info 2>/dev/null | grep 'Storage Driver'
   	`, registryURL, registryHost, registryHost, registryHost)},
	}
