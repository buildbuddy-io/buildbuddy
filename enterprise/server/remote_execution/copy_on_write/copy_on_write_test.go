...
func TestCOW_Basic(t *testing.T) {
	...
	require.NoError(t, err)
	s, err := copy_on_write.ConvertFileToCOW(ctx, env, path, chunkSizeBytes, dataDir, "", false, snaputil.ConvertToCOWConcurrency, lru)
	require.NoError(t, err)
...
func TestCOW_SparseData(t *testing.T) {
	...
	require.NoError(t, err)
	c, err := copy_on_write.ConvertFileToCOW(ctx, env, dataFilePath, chunkSize, outDir, "", false, snaputil.ConvertToCOWConcurrency, lru)
	require.NoError(t, err)
	...
	for _, test := range []struct {
		...
		t.Run(test.Name, func(t *testing.T) {
			for i := 0; i < 10; i++ {
				...
				lru, err := copy_on_write.GetSharedMmapLRU(dir)
				require.NoError(t, err)
				cow, err := copy_on_write.ConvertFileToCOW(ctx, env, src, chunkSize, dir, "", false, snaputil.ConvertToCOWConcurrency, lru)
				require.NoError(t, err)
...
func TestCOW_MmapLRUDoesNotDeadlock(t *testing.T) {
	...
	require.NoError(t, err)
	cow, err := copy_on_write.ConvertFileToCOW(ctx, env, path, chunkSize, chunkDir, "", false, snaputil.ConvertToCOWConcurrency, lru)
	require.NoError(t, err)
	...
	for _, test := range []struct {
		...
		b.Run(name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				lru, err := copy_on_write.GetSharedMmapLRU(chunkDir)
				require.NoError(b, err)
				cow, err := copy_on_write.ConvertFileToCOW(ctx, env, f.Name(), chunkSize, chunkDir, "", false, snaputil.ConvertToCOWConcurrency, lru)
				require.NoError(b, err)
	...
	for _, sparsenessRatio := range []float64{0, 0.1, 0.5, 0.9, 0.99, 1} {
		b.Run(fmt.Sprintf("%vSparse", sparsenessRatio), func(b *testing.B) {
			...
			for range b.N {
				...
				lru, err := copy_on_write.GetSharedMmapLRU(outputDir)
				require.NoError(b, err)
				_, err = copy_on_write.ConvertFileToCOW(context.Background(), nil, inputPath, chunkSizeBytes, outputDir, "", false, snaputil.ConvertToCOWConcurrency, lru)
				require.NoError(b, err)
...
func newMmap(t *testing.T) (*copy_on_write.Mmap, string) {
	...
	lru, err := copy_on_write.GetSharedMmapLRU(root)
	require.NoError(t, err)
	mmap, err := copy_on_write.NewMmapFd(ctx, env, root, false /*=dirty*/, int(f.Fd()), int(s.Size()), offset, snaputil.ChunkSourceLocalFile, "", false, lru)
	require.NoError(t, err)
	return mmap, path
}
