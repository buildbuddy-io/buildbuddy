...
func (h *Handler) handlePageFault(uffd uintptr, memoryStore *copy_on_write.COWStore, memoryMapping *GuestRegionUFFDMapping, faultingAddress uintptr) error {
	...
	if _, removed := h.removedAddresses[int64(faultingAddress)]; removed {
		delete(h.removedAddresses, int64(faultingAddress))
	...
	return h.copyDataFromMemorySnapshot(uffd, memoryStore, memoryMapping, faultingAddress)
}

// copyDataFromMemorySnapshot handles a page fault by copying data from the memory
// snapshot into the guest.
func (h *Handler) copyDataFromMemorySnapshot(uffd uintptr, memoryStore *copy_on_write.COWStore, memoryMapping *GuestRegionUFFDMapping, faultingAddress uintptr) error {
