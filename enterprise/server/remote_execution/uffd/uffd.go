...
func (h *Handler) handle(ctx context.Context, memoryStore *copy_on_write.COWStore) (err error) {
	...
	for {
		...
		//
		// By polling with a timeout if there are deferred page faults, the poll will return no events once the timeout
...
func (h *Handler) copyZeroes(uffd uintptr, faultingAddress uintptr, mapping *GuestRegionUFFDMapping) error {
	...
	if errno == unix.EAGAIN && addr > faultingAddress {
		...
		// as the actually faulting page was handled.
		return nil
	}
	if errno != 0 {
...
//
// The UFFDIO_COPY call wakes up the processes that triggered page faults on
// the copied pages (When a process attempts to access unallocated memory, it
// triggers a page fault and hangs until it has been resolved).
//
// Returns the number of bytes copied. This may be less than `size` if the copy
// stopped early - in that case err is non-nil, only threads faulting within
// the copied subrange have been woken, and the caller must resolve the rest of
// the range (see copyRange).
