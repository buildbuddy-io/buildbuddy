...
// ReliableStream implements a reliable operation stream.
//
...
func NewReliableStream(ctx context.Context, client repb.ExecutionClient, stream repb.Execution_ExecuteClient, name string) *ReliableStream {
...
func (s *ReliableStream) Name() string {
	return s.name
}
...
func (s *ReliableStream) Recv() (*longrunning.Operation, error) {
...
// Message contains an operation along with its payload conveniently
// unmarshaled.
type Message struct {
...
// Unpack unmarshals all Any-serialized operation fields.
func Unpack(op *longrunning.Operation) (*Message, error) {
	msg := &Message{Operation: op}
