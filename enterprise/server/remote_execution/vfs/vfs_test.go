...
func TestLayoutUpdate(t *testing.T) {
	...
	log.Warningf("READ FILE FIRST CHECK")

	data, err := os.ReadFile(filepath.Join(fsPath, "foo/bar/newfile"))
	require.NoError(t, err)
	require.Equal(t, "new", string(data))

	log.Warningf("NEW INPUT TREE WITH SAME FILE DIFFERENT CONTENT")

	newTree = createInputTree(t, ctx, env, map[string]string{
		"foo/bar/newfile": "changedcontent",
	})
	tf, err = dirtools.NewTreeFetcher(t.Context(), env, "", repb.DigestFunction_SHA256, newTree, &dirtools.DownloadTreeOpts{})
	require.NoError(t, err)
	_, err = tf.Start()
	...
	log.Warningf("READ FILE SECOND CHECK -- AFTER THIS THE CONTENTS SHOULD HAVE CHANGED")

	data, err = os.ReadFile(filepath.Join(fsPath, "foo/bar/newfile"))

	log.Warningf("READ FILE SECOND CHECK DONE")

	require.NoError(t, err)
	require.Equal(t, "changedcontent", string(data))
}
