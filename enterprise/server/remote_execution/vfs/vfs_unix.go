...
func (n *Node) resetCachedAttrs() {
	n.mu.Lock()
	defer n.mu.Unlock()
	log.CtxInfof(n.vfs.rpcCtx, "resetting cached attrs for %q", n.relativePath())
	n.cachedAttrs = nil
}
...
func (f *remoteFile) Fsyncdir(ctx context.Context, flags uint32) syscall.Errno {
	log.CtxWarningf(f.ctx, "FSYNCDIR %q", f.path)
	return fs.OK
}
...
func (n *Node) Create(ctx context.Context, name string, flags uint32, mode uint32, out *fuse.EntryOut) (node *fs.Inode, fh fs.FileHandle, fuseFlags uint32, errno syscall.Errno) {
	...
	if n.vfs.verbose {
		log.CtxDebugf(n.vfs.rpcCtx, "Create %q FLAGS %d", filepath.Join(n.relativePath(), name), flags)
	}
...
func (n *Node) Getattr(ctx context.Context, f fs.FileHandle, out *fuse.AttrOut) syscall.Errno {
	...
	if attrs != nil && rsp.Attrs.Size != attrs.Size && !n.IsDir() {
		log.CtxWarningf(n.vfs.rpcCtx, "Getattr %q size mismatch %d vs %d", n.relativePath(), attrs.Size, rsp.Attrs.Size)
	}

	if attrs == nil && cachable {
		attrs = rsp.GetAttrs()
		n.mu.Lock()
