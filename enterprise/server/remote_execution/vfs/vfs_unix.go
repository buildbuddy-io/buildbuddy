...
func New(vfsClient vfspb.FileSystemClient, mountDir string, options *Options) *VFS {
	...
	root := &Node{vfs: vfs}
	vfs.root = root
	vfs.nodesByInodeID[rootInodeId] = &vfs.root.Inode
	return vfs
}
...
func (vfs *VFS) PrepareForTask(ctx context.Context, taskID string, invalidatedInodeIDs []uint64) error {
	vfs.mu.Lock()
	...
	start := time.Now()
	numInvalidated := 0
	for _, invalidatedInodeID := range invalidatedInodeIDs {
		if vfs.verbose {
