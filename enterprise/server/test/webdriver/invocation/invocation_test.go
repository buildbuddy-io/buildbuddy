...
func TestAuthenticatedInvocation_Local_LogUploadEnabled(t *testing.T) {
	wt := webtester.New(t)
...
func TestAuthenticatedInvocation_Dev_LogUploadEnabled(t *testing.T) {
	wt := webtester.New(t)

	appURL := "https://app.buildbuddy.dev"
	workspacePath := testbazel.MakeTempWorkspace(t, map[string]string{
		"WORKSPACE": "",
		"BUILD":     `genrule(name = "a", outs = ["a.sh"], cmd_bash = "touch $@")`,
	})
	buildArgs := []string{
		...
		"--bes_backend=grpcs://remote.buildbuddy.dev",
	}

	// Log in and get the build flags needed for BuildBuddy, including API key
	webtester.Login(wt, appURL, "bbwebdriver@gmail.com", "bbwebdriver!00")
	buildbuddyBuildFlags := webtester.GetBazelBuildFlags(wt, appURL, webtester.WithEnableCache)
	t.Log(buildbuddyBuildFlags)
	buildArgs = append(buildArgs, buildbuddyBuildFlags...)
	...
	//
	//// Make sure we can view the invocation while logged in
	//wt.Get(appURL + "/invocation/" + result.InvocationID)
	//
	//details := wt.Find(".details").Text()
	//
	//assert.Contains(t, details, "Succeeded")
