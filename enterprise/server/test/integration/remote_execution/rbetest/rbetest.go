...
// DisconnectExecutor simulates the executor process exiting: its scheduler
// registration stream is torn down with no further coordination with the
// server, and the test env stops tracking it. Use it after BeginShutdown and
// WaitForShutdown to model an executor that leaves as soon as its own graceful
// shutdown completes, whether or not the scheduler is done with it. Unlike
// RemoveExecutor this does not wait for the scheduler's view of the pool to
// settle, since an executor that already handed back its work has left the
// pool before this point.
