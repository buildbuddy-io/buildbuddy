...
type fixedNodeTaskRouter struct {
	...
	gate chan struct{}
	// Number of RankNodes calls released by their context being done rather
	// than by Unblock.
	releasedByContext atomic.Int32
}
...
// ReleasedByContext returns how many RankNodes callers held by Block were
// released by their context being done rather than by Unblock.
func (f *fixedNodeTaskRouter) ReleasedByContext() int {
	return int(f.releasedByContext.Load())
}
...
// TestTaskReservationsNotLostWithMultipleExecutorsShuttingDown reproduces the
// way queued task reservations were orphaned during the 2026-09-09 arm64
// executor scale-down.
//
...
// registration stream handler, on that stream's context. The executor process
// exits as soon as its own graceful shutdown finishes, which cancels the
// stream context and drops whatever the scheduler had not re-enqueued yet. In
// production the scheduler got about 12 seconds per executor and dropped the
// tail of every hand-back list. The task router gate below stalls the
// scheduler at the same point so the executors can go away deterministically.
func TestTaskReservationsNotLostWithMultipleExecutorsShuttingDown(t *testing.T) {
	...
	// cancels the stream contexts the scheduler is re-enqueueing on, which
	// also releases the router gate for those callers.
	for _, e := range doomed {
		e.WaitForShutdown()
		rbe.DisconnectExecutor(e)
	}
	require.Eventually(t, func() bool { return taskRouter.ReleasedByContext() == len(doomed) }, 10*time.Second, 10*time.Millisecond,
		"the scheduler should observe the executors' stream contexts being cancelled mid hand-back")
