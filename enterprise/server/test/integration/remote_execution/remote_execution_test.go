...
func TestActionMerging_CancelAfterTTLExpiry(t *testing.T) {
	...
	const (
		queuedExecutionTTL = 1 * time.Second // Redis minimum TTL
		claimDelay         = 3 * time.Second
		commandTimeout     = 10 * time.Second
	)

	flags.Set(t, "debug_executor_task_claim_delay", claimDelay)
	flags.Set(t, "remote_execution.action_merging_queued_execution_ttl", queuedExecutionTTL)
	...
	_ = rbe.Execute(cmd, &rbetest.ExecuteOpts{
		CheckCache:   true,
	...
	// the task (claimDelay > queuedExecutionTTL).
	time.Sleep(queuedExecutionTTL + 500*time.Millisecond)
