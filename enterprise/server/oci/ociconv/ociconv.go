...
func ConvertContainerToExt4FS(ctx context.Context, resolver *oci.Resolver, workspaceDir, containerImage string, creds oci.Credentials, useOCIFetcher bool, outputPath string) error {
	...
	cmd := exec.CommandContext(
		...
		// Firecracker test images are generated by unprivileged Bazel actions, which
		// fail when tar tries to recreate OCI device entries using mknod. It is safe
		// to omit entries under /dev because guest startup mounts devtmpfs over them.
		"--exclude", "dev/*",
	)
