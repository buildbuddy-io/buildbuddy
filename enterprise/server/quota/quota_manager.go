...
func fetchConfigFromDB(env environment.Env, namespace string) (map[string]*namespaceConfig, error) {
	...
	err := env.GetDBHandle().Transaction(ctx, func(tx interfaces.DB) error {
		...
		err := db.ScanRows(ctx, rq, func(ctx context.Context, tb *tables.QuotaBucket) error {
		...
		rq = tx.NewQuery(ctx, "").Raw(groupQueryStr, groupArgs...)
		err = db.ScanRows(ctx, rq, func(ctx context.Context, tg *tables.QuotaGroup) error {
