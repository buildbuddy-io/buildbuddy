...
func TestLoadQuotasFromFlagd(t *testing.T) {
	...
	t.Run("insert new bucket for first time", func(t *testing.T) {
		// Load a bucket for the first time
		err := qm.loadQuotasFromFlagd(ctx, "key1", "rpc:/namespace1")
		require.NoError(t, err)
	...
	t.Run("skip loading if bucket already exists", func(t *testing.T) {
		...
		// Should still be the same bucket instance
		bucket := qm.findBucket("rpc:/namespace1", "key1")
		assert.Equal(t, originalBucket, bucket)
	})

	t.Run("add new bucket to existing namespace without removing others", func(t *testing.T) {
		// Load another bucket for a different key in the same namespace
		err := qm.loadQuotasFromFlagd(ctx, "key2", "rpc:/namespace1")
		require.NoError(t, err)
		...
		// Both should have the same config since they're from the same namespace
	...
	t.Run("different namespaces are independent", func(t *testing.T) {
		// Load a bucket in a different namespace with the same key
		err := qm.loadQuotasFromFlagd(ctx, "key1", "rpc:/namespace2")
		require.NoError(t, err)

		// Get buckets from both namespaces
