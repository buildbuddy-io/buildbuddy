...
func (tb *testLimitingBucket) Config() tables.QuotaBucket {
	return *tb.config
}
...
func createTestLimitingBucket(env environment.Env, config *tables.QuotaBucket, limit int64) (Bucket, error) {
...
func TestQuotaFlagdBuckets(t *testing.T) {
	...
	customBucketCreator := func(env environment.Env, config *tables.QuotaBucket) (Bucket, error) {
		if alreadyCreated {
			return nil, status.AlreadyExistsError("bucket already created")
		}
		alreadyCreated = true
		return createTestLimitingBucket(env, config, 5)
	}
	...
	assert.Equal(t, "restrictRead", config.Name)
...
func TestBucketRowFromMap(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			bucket, err := bucketRowFromMap(tc.namespace, tc.bucketMap)
			if tc.wantError {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
