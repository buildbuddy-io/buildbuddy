...
type CASServerProxy struct {
	env    environment.Env
	local  content_addressable_storage_server.ContentAddressableStorageServer
...
func New(env environment.Env) (*CASServerProxy, error) {
	local, err := content_addressable_storage_server.NewContentAddressableStorageServer(env)
	if err != nil {
		return nil, err
	}
...
func (s *CASServerProxy) GetTree(req *repb.GetTreeRequest, stream repb.ContentAddressableStorage_GetTreeServer) error {
	err := s.local.GetTree(req, stream)
	if err == nil {
		return nil
	}
