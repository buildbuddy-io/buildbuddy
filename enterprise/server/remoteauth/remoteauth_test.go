...
func validJWT(t *testing.T, userID string) string {
	authctx := claims.AuthContextFromUserInfo(context.Background(), &claims.Claims{UserID: userID}, nil)
...
func TestAuthenticateGRPCRequest(t *testing.T) {
	...
	_, err := authenticator.AuthenticateGRPCRequest(
		context.Background(), true /*=acceptJWT*/)
	...
	_, err = authenticator.AuthenticateGRPCRequest(
		contextWithApiKey(t, "foo"), true /*=acceptJWT*/)
	require.True(t, status.IsInternalError(err))

	// Don't cache errors.
	fakeAuth.Reset().setNextJwt(t, jwt1)
	userInfo, err := authenticator.AuthenticateGRPCRequest(
		contextWithApiKey(t, "foo"), true /*=acceptJWT*/)
	require.NoError(t, err)
	...
	fakeAuth.Reset().setNextJwt(t, jwt2)
	userInfo, err = authenticator.AuthenticateGRPCRequest(
		contextWithApiKey(t, "bar"), true /*=acceptJWT*/)
	require.NoError(t, err)
	...
	fakeAuth.Reset().setNextJwt(t, jwt1)
	userInfo, err = authenticator.AuthenticateGRPCRequest(
		contextWithJwt(t, jwt3), true /*=acceptJWT*/)
	require.NoError(t, err)
	...
	fakeAuth.Reset().setNextJwt(t, jwt1)
	_, err = authenticator.AuthenticateGRPCRequest(
		contextWithJwt(t, "invalid-jwt"), true /*=acceptJWT*/)
	require.Error(t, err)
}
