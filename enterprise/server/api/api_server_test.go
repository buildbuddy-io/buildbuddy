...
func TestGetTargetWithRealData(t *testing.T) {
	...
	env, ctx := getEnvAndCtx(t, "user1")
	// This build event json files was generated by a single BUILD file with the following content
	//
	//   [
	//       genrule(
	//           name = "gen_test_{}".format(i),
	//           outs = ["gen_test_{}.sh".format(i)],
	//           cmd = """
	//           cat > $@ <<EOF
	//   #!/bin/bash
	//   echo "Hello from gen_test_{}"
	...
	//       sh_test(
	//           name = "test_{}".format(i),
	//           srcs = ["gen_test_{}.sh".format(i)],
	//       )
	//       for i in range(NUM)
	//   ]
	//
	// where NUM value is 1000 or 20_000, which creates N build targets and N test targets.
	// The events is then streamed through BuildBuddy server to get sensitive redacted.
	// The invocation was built with Remote Cache enabled so that we get bytestream URIs
	// instead of local file:// URIs.
	for _, tc := range []struct {
		...
		t.Run(tc.testDataFile, func(t *testing.T) {
			iid := streamBuildFromTestData(t, env, tc.testDataFile)

			s := NewAPIServer(env)
			targetResp, err := s.GetTarget(ctx, &apipb.GetTargetRequest{Selector: &apipb.TargetSelector{InvocationId: iid}})
			require.NoError(t, err)
			require.NotNil(t, targetResp)
			require.Equal(t, tc.expectedCount, len(targetResp.GetTarget()))
...
func streamBuildFromTestData(t testing.TB, te *testenv.TestEnv, testDataFile string) string {
	...
	defer b.Close()
	fileReader = b
	if strings.HasSuffix(testDataFile, ".zst") {
