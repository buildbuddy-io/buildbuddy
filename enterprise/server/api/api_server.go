...
func (s *APIServer) Query(ctx context.Context, req *apipb.QueryRequest) (*apipb.QueryResponse, error) {
	...
	// Convert API proto to query service proto
	qReq := &querypb.QueryRequest{
		Repo:                     req.GetRepo(),
		Branch:                   req.GetBranch(),
		CommitSha:                req.GetCommitSha(),
		Query:                    req.GetQuery(),
		...
		Env:                      req.GetEnv(),
		PlatformProperties:       req.GetPlatformProperties(),
		RemoteHeaders:            req.GetRemoteHeaders(),
		Timeout:                  req.GetTimeout(),
		UseSystemGitCredentials:  req.GetUseSystemGitCredentials(),
		SkipAutoCheckout:         req.GetSkipAutoCheckout(),
		BazelFlags:               req.GetBazelFlags(),
	}
	
	// Call query service
	qRsp, err := qs.Query(ctx, qReq)
	if err != nil {
		return nil, err
	}
	
	// Convert query service proto to API proto
	return &apipb.QueryResponse{
		InvocationId: qRsp.GetInvocationId(),
		Result:       qRsp.GetResult(),
	}, nil
}
