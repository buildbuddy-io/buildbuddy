...
type timelineStatsRow struct {
	...
	DownloadQuantiles     []int64 `gorm:"type:int64[]"`
	UploadQuantiles       []int64 `gorm:"type:int64[]"`
}
...
func (r *timelineStatsRow) toSummaryProto() *expb.ExecutionTimelineSummary {
	return &expb.ExecutionTimelineSummary{
		...
		DownloadedBytesP10:   r.DownloadQuantiles[0],
		DownloadedBytesP50:   r.DownloadQuantiles[1],
		DownloadedBytesP90:   r.DownloadQuantiles[2],
		UploadedBytesTotal:   r.UploadedBytesTotal,
		UploadedBytesP10:     r.UploadQuantiles[0],
		UploadedBytesP50:     r.UploadQuantiles[1],
		UploadedBytesP90:     r.UploadQuantiles[2],
	}
}
...
func (s *ExecutionSearchService) queryTimelineExecutions(ctx context.Context, req *expb.GetExecutionTimelineRequest, groupID string) ([]*schema.Execution, error) {
	q := query_builder.NewQuery(`
		SELECT invocation_uuid, action_digest_hash, worker_start_timestamp_usec, worker_completed_timestamp_usec, cpu_nanos, peak_memory_bytes, file_download_size_bytes, file_upload_size_bytes, action_mnemonic, os, arch, output_path
		FROM "Executions"
	`)
	if err := s.addTimelineWhereClauses(q, groupID, req); err != nil {
		return nil, err
