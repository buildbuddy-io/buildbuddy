...
func (s *SchedulerServer) sampleUnclaimedTasks(ctx context.Context, count int, nodePoolKey nodePoolKey, node *scpb.ExecutionNode) ([]*persistedTask, error) {
	...
	//
	// TODO: sample only from tasks that fit. Currently if there are a lot of
	// tasks that don't fit, it's likely we'll return no tasks here, even if
	// there are some tasks that do fit.
