...
// RedactBuildBuddyEmployee redacts BuildBuddy employee information from audit
// log entries when viewed by non-impersonating users. This protects employee
// privacy by hiding their email addresses and replacing with "Buildbuddy Admin",
// and masking the client IP address.
func RedactBuildBuddyEmployee(entry *alpb.Entry, userEmail string, isImpersonating bool) {
	if isImpersonating {
		return
	}
...
func (l *Logger) GetLogs(ctx context.Context, req *alpb.GetAuditLogsRequest) (*alpb.GetAuditLogsResponse, error) {
	...
	err = db.ScanEach(rq, func(ctx context.Context, e *schema.AuditLog) error {
		...
		RedactBuildBuddyEmployee(entry, e.AuthUserEmail, u.IsImpersonating())
