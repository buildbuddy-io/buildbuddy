...
var (
	...
	minBlobSize      = flag.Int64("min_blob_size", 0, "Minimum size of blobs to keep from blob dir")
	maxBlobSize      = flag.Int64("max_blob_size", 1e9, "Maximum size of blobs to keep from blob dir")
	...
	decompressOnRead = flag.Bool("decompress_on_read", true, "Whether to decompress on read.")
	showBlobStats    = flag.Bool("show_blob_stats", false, "If set, compute blob stats for the blobs in blob_dir.")
	zstdLib          = flag.String("zstd_lib", "klauspost", "ZSTD compression libary to use.")
...
func writeBlobsForReading() []*repb.Digest {
	...
	for i := 0; uint(i) < *concurrency; i++ {
		blob := randomBlob()
		_, err := cachetools.UploadFromReader(ctx, bsClient, blob.ResourceName, bytes.NewReader(blob.Data))
		if err != nil {
			log.Fatalf("Error pre-writing blob %q: %s", blob.ResourceName.GetDigest().GetHash(), err)
		}
		digests = append(digests, blob.ResourceName.GetDigest())
	}
	return digests
}
...
func randomBlob() *Blob {
	...
	// Apply compressor but keep uncompressed digest (per protocol)
...
func writeDataFunc(mtd *desc.MethodDescriptor, cd *runner.CallData) []byte {
	blob := randomBlob()
	uploadName, err := blob.ResourceName.UploadString()
	if err != nil {
...
func readDataFunc(mtd *desc.MethodDescriptor, cd *runner.CallData) []byte {
	...
	r := digest.NewResourceName(randomDigest, *instanceName)
	r.SetCompressor(getCompression())
...
func streamRecvFunc(msg *dynamic.Message, err error) error {
	...
	if !*compressBlobs || !*decompressOnRead {
		return nil
	}
...
func readBlobDir(path string) []*blobFile {
	...
	for _, entry := range entries {
		...
		if d.GetSizeBytes() < *minBlobSize {
			continue
		}
		if d.GetSizeBytes() > *maxBlobSize {
			continue
		}
...
func main() {
	...
	md["x-buildbuddy-cache-zstd-lib"] = *zstdLib
