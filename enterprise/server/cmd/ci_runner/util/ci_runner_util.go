...
var (
	RecycledCIRunnerMaxWait = flag.Duration("remote_execution.workflows_runner_recycling_max_wait", 3*time.Second, "Max duration that a ci_runner task should wait for a warm runner before running on a potentially cold runner.")
	CIRunnerDefaultTimeout  = flag.Duration("remote_execution.workflows_default_timeout", 8*time.Hour, "Default timeout applied to all ci runners.")
)
