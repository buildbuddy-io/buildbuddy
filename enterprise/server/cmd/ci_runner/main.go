...
var (
	...
	pushedBranch  = flag.String("pushed_branch", "", "Branch name to report statuses for.")
	commitSHA     = flag.String("commit_sha", "", "Commit SHA to report statuses for.")
	targetRepoURL = flag.String("target_repo_url", "", "URL of the target repo. If different from pushed_repo_url, indicates a fork (`pushed_repo_url`) is being merged into this repo.")
	targetBranch  = flag.String("target_branch", "", "Branch to check out in the target repo.")
...
func (ws *workspace) fetch(ctx context.Context, remoteURL string, refs []string) error {
	...
	fetchArgs := append([]string{"-c", "credential.helper=", "fetch", "--filter=blob:none", "--force", remoteName}, refs...)
