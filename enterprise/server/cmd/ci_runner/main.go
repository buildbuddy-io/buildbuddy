...
var (
	...
	recordRunMetadata  = flag.Bool("record_run_metadata", false, "")
...
func (ar *actionRunner) Run(ctx context.Context, ws *workspace) error {
	...
	for i, bazelCmd := range ar.action.BazelCommands {
		...
		// extract run options (i.e. args) from the generated run script.
		runScript := ""
...
func uploadRunfiles(ctx context.Context, workspaceRoot, runfilesDir string) ([]*bespb.File, error) {
	...
	for _, d := range missingDigests {
		...
		runFiles = append(runFiles, &bespb.File{
			Name: relPath,
			File: &bespb.File_Uri{
				Uri: fmt.Sprintf("bytestream://remote.buildbuddy.io/%s", digest.NewResourceName(d, *remoteInstanceName).DownloadString()),
			},
		})
	}
