...
func run() error {
	...
	if err := writeHomeBazelrc(); err != nil {
		return fmt.Errorf("provision ~/.bazelrc: %w", err)
	}
...
func writeHomeBazelrc() error {
	lines := []string{
		"# This file is generated by BuildBuddy. DO NOT EDIT.",
		// Enable BuildBuddy BES by default.
		"build --bes_backend=" + os.Getenv("BUILDBUDDY_BES_BACKEND"),
		"build --bes_results_url=" + os.Getenv("BUILDBUDDY_BES_RESULTS_URL"),
		// Set up configs for BuildBuddy endpoints.
		"build:buildbuddy_bes_backend --bes_backend=" + os.Getenv("BUILDBUDDY_BES_BACKEND"),
		"build:buildbuddy_bes_results_url --bes_results_url=" + os.Getenv("BUILDBUDDY_BES_RESULTS_URL"),
		"build:buildbuddy_remote_executor --remote_executor=" + os.Getenv("BUILDBUDDY_REMOTE_EXECUTOR"),
		"build:buildbuddy_remote_cache --remote_cache=" + os.Getenv("BUILDBUDDY_REMOTE_CACHE"),
		"build:buildbuddy_experimental_remote_downloader --experimental_remote_downloader=" + os.Getenv("BUILDBUDDY_REMOTE_CACHE"),
		// Disable commit status reporting since we're already publishing a
		// status via the GitHub workflow.
	...
	f, err := os.Create(filepath.Join(home, ".bazelrc"))
	if err != nil {
		return err
	}
	defer f.Close()
	if _, err = io.WriteString(f, strings.Join(lines, "\n")); err != nil {
		return err
	}
	return nil
}
