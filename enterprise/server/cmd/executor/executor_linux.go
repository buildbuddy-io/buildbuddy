...
func setupCgroups() (string, error) {
	...
	if !*childCgroupsEnabled {
		if startingCgroup == cgroup.RootName {
			return startingCgroup, nil
		}
		ociEnabled, err := flagutil.GetDereferencedValue[bool]("executor.enable_oci")
		if err != nil {
			return "", fmt.Errorf("failed to look up executor.enable_oci flag: %w", err)
		}
		// TODO: this warning feels a bit obscure - figure out better defaults
		// for child cgroups and remove it?
		if ociEnabled && os.Getuid() != 0 {
			log.Warningf("OCI rootless mode in non-root cgroup requires child cgroups to be enabled; ignoring -executor.child_cgroups_enabled=false setting")
		} else {
			return startingCgroup, nil
		}
	}
