...
func New(env environment.Env) (repb.ActionCacheClient, error) {
	...
	copyOp := operators.NewDirectFlushOperator(func(ctx context.Context, groupID string, b *batch_operator.DigestBatch) error {
		return handleCopy(ctx, groupID, b, routingService)
	...
	copyOp.Start(env.GetHealthChecker())
	readOp := operators.NewDirectFlushOperator(func(ctx context.Context, groupID string, b *batch_operator.DigestBatch) error {
		return handleRead(ctx, groupID, b, routingService)
	}, env.GetAuthenticator())
	...
	readOp.Start(env.GetHealthChecker())
