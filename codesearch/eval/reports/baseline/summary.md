...
|---|---|---|---|---|---|---|---|---|---|---|
| symbol | 25 | 25 | 8% | 28% | 0.22 | 80% | 96% | 0.88 | 0.36 | 28% |
| keyword | 20 | 20 | 25% | 70% | 0.42 | 70% | 90% | 0.78 | 0.46 | 65% |
| phrase | 15 | 15 | 27% | 80% | 0.45 | 40% | 80% | 0.57 | 0.33 | 67% |
| regex | 15 | 6 | 0% | 17% | 0.06 | 50% | 83% | 0.62 | 0.18 | 33% |
| filter | 10 | 5 | 20% | 20% | 0.25 | 60% | 100% | 0.80 | 0.32 | 30% |
| ranking | 15 | 0 | - | - | - | - | - | - | 0.16 | 20% |
| **total** | 100 | 71 | 17% | 49% | 0.31 | 65% | 90% | 0.76 | 0.32 | 41% |
...
- `WaitGroup` (sym-waitgroup): local rank 7, google rank 1, target src/sync/waitgroup.go
- `func ListenAndServe` (sym-listenandserve): local rank 9, google rank 1, target src/net/http/server.go
- `NewScanner` (sym-newscanner): local rank 11, google rank 1, target src/bufio/scan.go
- `gopanic` (sym-gopanic): local rank 8, google rank 1, target src/runtime/panic.go
- `mallocgc` (sym-mallocgc): local rank 10, google rank 1, target src/runtime/malloc.go
- `WithCancel` (sym-withcancel): local rank 6, google rank 1, target src/context/context.go
- `Goexit` (sym-goexit): local rank 33, google rank 1, target src/runtime/panic.go
...
- `GOMAXPROCS` (sym-gomaxprocs): local rank 9, google rank 1, target src/runtime/debug.go
- `CopyBuffer` (sym-copybuffer): local rank 6, google rank 1, target src/io/io.go
- `stopTheWorld` (sym-stoptheworld): local rank 9, google rank 1, target src/runtime/proc.go
- `QuoteMeta` (sym-quotemeta): local rank 48, google rank 1, target src/regexp/regexp.go
...
- `func ReadAll` (sym-readall): local rank 19, google rank 2, target src/io/io.go
- `func Fields` (sym-fields): local rank miss, google rank 9, target src/strings/strings.go
- `context cancel` (kw-context-cancel): local rank 6, google rank 1, target src/context/context.go
- `tls handshake client` (kw-tls-handshake-client): local rank 18, google rank 1, target src/crypto/tls/handshake_client.go
...
- `channel send receive` (kw-channel-send-receive): local rank 14, google rank 3, target src/runtime/chan.go
- `"concurrent map read and map write"` (phr-concurrent-map): local rank 7, google rank 1, target src/runtime/map_noswiss.go
- `"declared and not used"` (phr-declared-not-used): local rank 44, google rank 14, target src/cmd/compile/internal/types2/stmt.go
...
- `lang:go file:print.go Fprintf` (flt-print-fprintf): local rank 13, google rank 1, target src/fmt/print.go
- `case:yes Printf` (flt-case-printf): local rank miss, google rank 2, target src/fmt/print.go
- `file:json decode` (flt-json-decode): local rank 29, google rank 2, target src/encoding/json/decode.go
