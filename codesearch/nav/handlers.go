	...
	for _, ticket := range req.GetTicket() {
		...
		for _, loc := range locs {
			set.Definition = append(set.Definition, &kxpb.CrossReferencesReply_RelatedAnchor{
				Anchor: &kxpb.Anchor{
					Parent: fileTicket(loc.Repo, loc.Path),
...
func Documentation(ctx context.Context, r *index.Reader, req *srpb.ExtendedDocumentationRequest) (*srpb.ExtendedDocumentationReply, error) {
	...
	reply.Definition = &kxpb.CrossReferencesReply_RelatedAnchor{
		Anchor: &kxpb.Anchor{
			Parent:  fileTicket(d.Repo, d.Path),
	...
	for _, ticket := range req.GetTickets() {
		...
		for _, rf := range refs {
			reply.References = append(reply.References, &kxpb.CrossReferencesReply_RelatedAnchor{
				Anchor: &kxpb.Anchor{
					Parent:  fileTicket(rf.Repo, rf.Path),
					Span:    span(rf.Start, rf.End),
					Snippet: rf.Snippet,
				},
			})
...
// fileTicket formats a file's ticket: the repository as the authority and the
// repo-relative path as the query, e.g. tree-sitter://buildbuddy?path=a/b.go.
// The frontend extracts the path (it splits on "?path="); the repo makes the
// ticket self-describing and is where repo-scoped resolution will hook in.
func fileTicket(repo, path string) string {
	return annotations.Scheme + "://" + repo + "?path=" + path
}
