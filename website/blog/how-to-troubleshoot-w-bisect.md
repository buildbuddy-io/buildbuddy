...
$ bazelisk --bisect=8.0.1..8.1.0 --output_base=$(mktemp -d) test --config=remote-minimal //...
```
...
Because the issue was related to external dependencies download, we want to have a clean workspace for each test run to avoid local cache affecting the test result. Instead of running `bazel clean --expunge` manually, we leverage `--output_base=$(mktemp -d)` here to create a temporary directory for each bisect run instead.
...
```bash
$ bazelisk --bisect=8.0.1..8.1.0 --output_base=$(mktemp -d) query --config=remote-minimal @rules_cc//:all
```

Note that using a separate output base directory will spin up a new Bazel JVM process each time. So if the bisect range is large, you may run into memory issues.
...
(
  cd ~/work/buildbuddy-io/buildbuddy
  STDERROUT=$(bazel 2>&1 --output_base=$(mktemp -d) query --repository_cache='' --config=remote-minimal @bazel_features//:all)
  BAZEL_EXIT_CODE=$?
...
Next, as we are using a new `--output_base=` directory each time, we want to make sure that repeated runs of this script won't bloat our laptop with many Bazel JVM processes. So we set up a `cleanup()` function to shut down Bazel after each run.
...
We won't be diving into the details of the specific issue in this blog post,
but if you are interested, you can read more about it via [the fixed PR #9999](https://github.com/bazelbuild/bazel/pull/9999).
Many thanks to Fabian for the swift fix!
...
We are also not expecting the build to fail because of external factors, so no need for a separate `output_base`.
