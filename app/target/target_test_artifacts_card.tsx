...
export default class TargetTestArtifactsCardComponent extends React.Component<Props, State> {
  state: State = {
    loading: false,
    cacheEnabled: false,
    manifest: null,
  ...
  maybeFetchOutputManifest() {
    let testLogUrl = this.props.files.find(
      (file: build_event_stream.File) => true && file.name?.endsWith("outputs.zip")
    )?.uri;
