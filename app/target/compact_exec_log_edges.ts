...
export function normalizeExecConfiguredPath(path: string): string {
  // Exec-configured outputs may include an ST hash segment; strip it so we can
  // match the canonical `*-exec/` artifact path recorded elsewhere in the log.
  return path.replace(/-exec-ST-[^/]+\//, "-exec/");
}
...
function resolveSymlinkOutputArtifactId(actionId: string, state: IndexBuildState): number | undefined {
  ...
  const normalizedPath = normalizeExecConfiguredPath(symlinkAction.outputPath);
  return state.pathToArtifactId.get(symlinkAction.outputPath) || state.pathToArtifactId.get(normalizedPath);
}
