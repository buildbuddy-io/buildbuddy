...
export default class SetupCodeComponent extends React.Component {
  ...
  getCacheOptions() {
    if (this.state.cache == "read") return <span>build --noremote_upload_local_results <span className="comment"># Uploads logs & artifacts without writing to cache</span></span>
    if (this.state.cache == "top") return <span>build --remote_download_toplevel <span className="comment"># Helps remove network bottleneck if caching is enabled</span></span>
    return "";
  }
        ...
        <div className="setup-controls">
          ...
          {this.isAuthEnabled() && <span className="group">
            ...
            <input id="auth-key"
              ...
              onChange={this.handleInputChange.bind(this)}
              value="key" name="auth" type="radio" />
            <label htmlFor="auth-key">API Key</label>
            {this.isCertEnabled() && <span className="group-section">
              <input id="auth-cert"
          ...
          {this.isCacheEnabled() && <span>
            <input id="cache"
              ...
              name="cacheChecked"
              type="checkbox" />
            <label htmlFor="cache"><span>Enable cache</span></label>
          </span>}

          {this.state.cacheChecked && <span className="group">
            <input id="cache-full"
              checked={this.state.cache == "full"}
              onChange={this.handleInputChange.bind(this)}
              value="full" name="cache" type="radio" />
            <label htmlFor="cache-full">Full cache</label>
            <input id="cache-read"
              checked={this.state.cache == "read"}
              onChange={this.handleInputChange.bind(this)}
              value="read" name="cache" type="radio" />
            <label htmlFor="cache-read">Read only</label>
            <input id="cache-top"
              checked={this.state.cache == "top"}
              onChange={this.handleInputChange.bind(this)}
              value="top" name="cache" type="radio" />
