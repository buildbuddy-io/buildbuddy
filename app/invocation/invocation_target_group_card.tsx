                ...
                {targets.map((target) => (
                  <Link className="target-row" href={this.getTargetURL(target)}>
                    <div title={targetTitleAttr(target)} className="target">
                      ...
                      {target.testSummary && target.testSummary.totalNumCached > 0 && (
                        <span className="target-cache-status">
                          {target.testSummary.totalNumCached === target.testSummary.totalRunCount
                            ? "Cached"
                            : `${target.testSummary.totalNumCached} / ${target.testSummary.totalRunCount} runs cached`}
                        </span>
                      )}
