...
import { addDays, isSameDay, MONTH_NAMES, startOfDay, startOfMonth, WEEKDAY_NAMES } from "../../util/date";
    ...
    return (
      <div className="date-range-picker">
        <div className="preset-picker">
          {LAST_N_DAYS_OPTIONS.map((days) => {
            const startDate = startOfDay(addDays(now, 1 - days));
            const selected =
              !!range.startDate && isSameDay(range.startDate, startDate) && isSameDay(range.endDate ?? now, now);
            const onSelect = () => this.props.onChange({ startDate, endDate: now, days });
            return (
              <div
                key={days}
                role="button"
...
class DateInput extends React.Component<DateInputProps, DateInputState> {
  ...
  private commit() {
    ...
    const parsed = new Date(this.state.text);
...
function fillClass(base: string, fill: { start: boolean; end: boolean }): string {
  return `${base}${fill.start ? " round-l" : ""}${fill.end ? " round-r" : ""}`;
}
