...
// bb commands are expected to look like:
// bb <cli flags> <bb command> <bazel command> <bazel flags> -- <exec flags>
// Ex. bb --verbose --remote_runner_target=remote.buildbuddy.dev remote build //... --jobs=100 -- --my_go_app_flag=hello
//
// Cli flags: Affect configuration of the bb cli executable.
//   - Ex. --verbose and --remote_runner_target
//
// Bazel flags: Flags passed through to the bazel server.
//   - Ex. --jobs
//
// Exec flags: Flags passed through to the executable that bazel runs
//   - Ex. --my_go_app_flag
...
// Ex. bb remote build //... <--bazel_args> -- <--executable_args>
...
// Splits out flags that affect the bb cli executable. These should be set before
// the bb cli command.
// Ex. `bb <--cli_args> execute`, bb <--cli_args> remote build //...
// Returns cli args and the original args with the cli args removed
func SplitCLIArgs(args []string) ([]string, []string) {
	splitIndex := 0
	for splitIndex < len(args) {
		if !strings.HasPrefix(args[splitIndex], "--") {
			break
		}
		splitIndex++
	}
	cliArgs := args[:splitIndex]
	remainingArgs := args[splitIndex:]
	return cliArgs, remainingArgs
}
