...
`,
	...
	p, err := GetParser()
	require.NoError(t, err)
	args, err := p.ParseArgs([]string{"build", "--config=a"})
	require.NoError(t, err)
	configs, defaultConfig, err := p.consumeAndParseRCFiles(args, ws)
	require.NoError(t, err)
	_, err = args.ExpandConfigs(configs, defaultConfig)
	require.Error(t, err)

	assert.Contains(t, err.Error(), "circular --config reference detected: a -> b -> c -> a")

	args, err = p.ParseArgs([]string{"build", "--config=d"})
	require.NoError(t, err)
	configs, defaultConfig, err = p.consumeAndParseRCFiles(args, ws)
	require.NoError(t, err)
	_, err = args.ExpandConfigs(configs, defaultConfig)
