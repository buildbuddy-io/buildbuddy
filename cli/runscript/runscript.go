...
func Configure(args []string, effectiveArgs []string) (newArgs []string, scriptPath string, err error) {
	if arg.GetCommand(effectiveArgs) != "run" {
		return args, "", nil
	}
	...
	existingScript := arg.Get(effectiveArgs, "script_path")
	if existingScript != "" {
		return args, "", nil
	}
