...
// forwarded arguments to be passed to bazel. The plugin can read and write that
// file to modify the args (most commonly, just appending to the file), which
// will then be fed to the next plugin in the pipeline, or passed to Bazel if
// this is the last plugin. Plugins can also read the resolved arg view from the
...
func (p *Plugin) PreBazel(bazelArgs *parser.BazelArgs, execArgs []string) (*parser.BazelArgs, []string, error) {
	...
	if err := bazelArgs.Update(newArgs); err != nil {
		return nil, nil, err
	}
