...
func TestToolRunfilesEmptyFilesDiffer(t *testing.T) {
	...
	d := sd.GetModified().Diffs[0]
	require.IsType(t, &spawn_diff.Diff_InputPaths{}, d.Diff)
	assert.Subset(t, d.GetInputPaths().GetOldOnly(), []string{"__init__.py", "_main/pkg/__init__.py"})
	assert.NotContains(t, d.GetInputPaths().GetOldOnly(), "_main/pkg/lib/__init__.py")
