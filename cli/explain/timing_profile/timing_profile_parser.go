...
// Duration wrapper so that it can be marshalled to a human-readable string in JSON,
// instead of the raw nanosecond count time.Duration produces by default.
type Duration time.Duration

func (d Duration) String() string {
	// Truncate to 0.1s to reduce noise, but keep full precision for sub-0.1s
	// values so small durations don't collapse to "0s".
	td := time.Duration(d)
	if td >= 100*time.Millisecond {
		td = td.Truncate(100 * time.Millisecond)
	}
	return td.String()
}
...
func (p *timingProfileParser) addSpan(event trace_events.Event) {
	...
	p.spans = append(p.spans, TimingSpan{
		...
		Duration: Duration(time.Duration(event.Duration) * time.Microsecond),
	})
}
...
func (p *timingProfileParser) wallTimePercent(d Duration) float64 {
...
func (p *timingProfileParser) addDuration(category string, durationUsec int64) {
	...
	aggregate.Duration += Duration(time.Duration(durationUsec) * time.Microsecond)
	aggregate.Count++
}
