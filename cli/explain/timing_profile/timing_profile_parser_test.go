...
func TestParseTimingProfile(t *testing.T) {
	...
	require.Equal(t, Duration(64*time.Microsecond), profile.TotalDuration)

	byCategory := aggregateDurations(profile.DurationByCategory)
	require.Equal(t, Duration(17*time.Microsecond), byCategory["remote action execution"])
	require.Equal(t, Duration(35*time.Microsecond), byCategory["build phase marker"])
	require.Equal(t, Duration(34*time.Microsecond), byCategory["action processing"])
	...
	require.Equal(t, Duration(13*time.Microsecond), profile.SlowestActions[0].Duration)
	...
	require.Equal(t, Duration(11*time.Microsecond), profile.SlowestActions[1].Duration)
...
func TestParseTimingProfile_Limit(t *testing.T) {
	...
	require.Equal(t, []Duration{
		Duration(13 * time.Microsecond),
		Duration(11 * time.Microsecond),
...
func TestDurationMarshalJSON(t *testing.T) {
	for _, tc := range []struct {
		...
		{Duration(9*time.Minute + 26*time.Second + 278*time.Millisecond), `"9m26.2s"`},
		{Duration(1500 * time.Millisecond), `"1.5s"`},
		{Duration(13 * time.Microsecond), `"13µs"`}, // sub-0.1s kept at full precision
	} {
...
func aggregateDurations(aggregates []TimingAggregate) map[string]Duration {
