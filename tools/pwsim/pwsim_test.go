...
var (
	...
	executorMemoryBytes       = flag.Int64("executor_memory_bytes", 65*(1024*1024*1024), "Assignable memory for each executor.")
	excessCapacityThreshold   = flag.Float64("executor_excess_capacity_threshold", 0.4, "Fraction of resources used where the executor will proactively request more work from the scheduler.")
...
func (s *Scheduler) Enqueue(ctx context.Context, task *Task) error {
	...
	if len(executors) != probesPerTask {
		panic(fmt.Sprintf("expected %d probes, got %d", probesPerTask, len(executors)))
	}
...
func (s *Scheduler) route(task *Task) []*RankedNode {
	...
	case "affinity_router":
		...
		return toRankedNodes(append([]*Executor{affinityNode}, executors[:min(2, len(executors))]...))
	...
	case "persistent_worker_router":
		...
		return toRankedNodes(executors[:min(3, len(executors))])
	...
	case "adaptive_subpools":
		...
		// Shuffle the subpool and select up to 3 probes
		rand.Shuffle(len(subpool), func(i, j int) {
			subpool[i], subpool[j] = subpool[j], subpool[i]
		})
		probes := subpool[:min(probesPerTask, len(subpool))]
...
func (s *Scheduler) routeRandomly(task *Task) []*RankedNode {
	...
	var out []*RankedNode
	for _, e := range executors[:min(3, len(executors))] {
		out = append(out, &RankedNode{
			Executor:  e,
			Preferred: false,
		})
...
func (p *RunnerPool) TryRecycle(r *Runner) {
	...
	shouldEvict := func() bool {
		return p.pausedRunnerCount()+1 > *runnerPoolMaxCount
	}
	if p.sim.runnerPoolTotalMemoryLimitBytes > 0 {
		shouldEvict = func() bool {
			return p.totalPausedRunnerMemoryBytes()+r.PeakMemoryBytes > p.sim.runnerPoolTotalMemoryLimitBytes
		}
	}
