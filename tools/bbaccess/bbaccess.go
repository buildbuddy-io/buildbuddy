...
var (
	...
	tunnelConfigPath = flag.String("tunnel_config", "", "Path to the tunnel config file (default: <user config dir>/buildbuddy/tunnel.yaml)")
)
...
func fetchCerts() {
	...
	cfg, _, err := tunnel.LoadConfig(*tunnelConfigPath)
	if err != nil {
	...
	// A running daemon reloads the certificate from disk on its next
	// registration, so rewriting the files above is the whole refresh
	// mechanism — there is nothing to signal.
	tunnel.PrintCredentialStatus(cfg)

	if *runTunnel {
		log.Infof("Starting the tunnel daemon. Interrupt to stop it.")
		if err := tunnel.RunDaemon(cfg, "" /*=apiKey*/); err != nil {
			log.Fatalf("Tunnel daemon: %s", err)
		}
	}
}

func fetchCert(ctx context.Context, server, homeDir, keyFile string, pub []byte, token string, cfg *config.Config) error {
	...
	// The tunnel credential is named after the server that issues it, so a
	// config with both dev and prod gateways can say which to use, and so the
	// files sit recognizably beside the SSH certs from the same server. Its
	// keypair is generated here on first use; only the public key is sent.
	...
	// The relay gateways this credential is for, and the zones each serves,
	// come from the same server and are stored beside the credential. A
	// server that sends none no longer has any: forget what it sent before.
	gateways := config.ServerGateways{}
	for _, gw := range tc.GetGateways() {
		sg := config.ServerGateway{Target: gw.GetTarget()}
		for _, z := range gw.GetZones() {
			sg.Zones = append(sg.Zones, config.ServerZone{Suffix: z.GetSuffix(), RewriteTo: z.GetRewriteTo()})
		}
