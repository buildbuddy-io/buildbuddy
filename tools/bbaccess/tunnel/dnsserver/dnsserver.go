...
//
// It never asks an upstream resolver: a name in a covered zone gets a fake IP
// straight away, and the real resolution happens on the gateway when the
// connection is made. That keeps prod's DNS private and makes the first query
// the moment we learn a developer is about to reach production — which is what
// triggers tunnel setup, well before the first packet.
package dnsserver
...
type Prewarmer interface {
	Prewarm(ctx context.Context, zone config.Zone)
}
...
func New(cfg *config.Config, table *fakeip.Table, warmer Prewarmer) *Server {
