// Package daemon runs the tunnel services including DNS, tun device packet
// interception and on-demand wireguard tunnel setup.
package daemon
...
// for changes so that changes written by `bbaccess` can be picked up
...
func Run(cfg *tunnelconfig.Config, creds tunnelmgr.Credentials) error {
...
func logZones(cfg *tunnelconfig.Config) {
	log.Infof("Covered zones:")
...
func PrintStatus(cfg *tunnelconfig.Config, names []string) {
	fmt.Printf("Config:\n")
...
// resolver.
func ResolverDomains(cfg *tunnelconfig.Config) []string {
	// We require all zones to live under the same suffix so that we can install
	// a single resolver rule instead of having to set one up per zone.
	out := []string{tunnelconfig.Parent}
