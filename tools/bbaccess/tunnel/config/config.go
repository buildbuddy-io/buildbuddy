// Package config describes which DNS zones the tunnel covers and which gateway
// serves each one.
//
...
// Zone maps a DNS suffix to the gateway that can reach names under it.
type Zone struct {
	// Suffix is the DNS suffix this zone claims, e.g. "prod.buildbuddy.io".
	Suffix string `yaml:"suffix"`

	// Gateway is the gRPC target of the gateway serving this zone.
	Gateway string `yaml:"gateway"`

	// Network is the gateway network name to register in. Usually empty.
	...
	//
	//	suffix:     svc.uswest1.buildbuddy.io
	//	rewrite_to: svc.cluster.local
	//
	// makes "otel-collector.monitor-dev.svc.uswest1.buildbuddy.io" reach the
	// us-west1 gateway as "otel-collector.monitor-dev.svc.cluster.local".
	// Unambiguous cross-cluster names on the workstation, ordinary Kubernetes
	// names on the wire.
	RewriteTo string `yaml:"rewrite_to,omitempty"`

	// Credential names the bbaccess-issued tunnel certificate to authenticate
	// with, matching a file written by bbaccess (e.g. "certs_buildbuddy_io").
	// Empty means use the only credential present, which is the common case.
	Credential string `yaml:"credential,omitempty"`

	// Audience overrides the gateway identity a credential is minted for. It
	// must match the gateway's --gateway.cert_auth.audience. Empty derives it
	// from Gateway's hostname, which is usually what you want: it binds the
	// credential to the host we believe we are talking to.
	Audience string `yaml:"audience,omitempty"`

	// Source is where the zone came from, for status output and error
	// messages: the config file, or a server's gateways file.
	Source string `yaml:"-"`
}
...
type Config struct {
	...
	// DNSListen is the address the local DNS server binds. Port 5533 avoids
	// mDNSResponder, which owns 5353 on macOS.
	...
	// FakeCIDR is the range fake IPs are allocated from. 198.18.0.0/16 is
	// RFC 2544 benchmarking space: reserved, never routed on the internet, and
	// not the CGNAT range that Tailscale claims.
	...
	// APIKey authenticates with gateways using a BuildBuddy API key instead of
	// a tunnel certificate. Employees should leave this unset and let bbaccess
	// issue a certificate; it exists for gateways with no tunnel CA configured.
...
//
// This matters more than it looks: `tunnel install` needs root, and macOS
// requires the daemon itself to run as root, yet both have to read the
// invoking user's config and credentials. os.UserConfigDir() follows $HOME,
// which sudo points at root — so without this, installing would silently
// configure the built-in default zones instead of the user's.
...
func Load(path string) (*Config, error) {
	...
	for i := range parsed.Zones {
		parsed.Zones[i].Source = filepath.Base(path)
	}
	cfg.Zones = parsed.Zones
	if parsed.DNSListen != "" {
...
// overlaps no other, so that every name belongs to exactly one gateway. An
// empty zone list is valid here; the daemon refuses to run without zones, but
// installing needs none.
func (c *Config) Validate() error {
	seen := make(map[string]Zone, len(c.Zones))
	for i, z := range c.Zones {
		s := normalize(z.Suffix)
		if s == "" {
			return fmt.Errorf("zone %d (%s): suffix is required", i, z.source())
		}
		if !UnderParent(s) {
			return fmt.Errorf("zone %q (%s): suffix must be under %s, the suffix routed to the tunnel", s, z.source(), Parent)
		}
		if strings.TrimSpace(z.Gateway) == "" {
			return fmt.Errorf("zone %q (%s): gateway is required", s, z.source())
		}
		for o, oz := range seen {
			if s == o || strings.HasSuffix(s, "."+o) || strings.HasSuffix(o, "."+s) {
				return fmt.Errorf("zone %q (%s) overlaps zone %q (%s)", s, z.source(), o, oz.source())
			}
		}
		seen[s] = z
	}
...
func (g ServerGateways) zones(credential, source string) []Zone {
	var out []Zone
...
type serverZones struct {
	...
	dir  string
	user []Zone               // zones from the config file, kept across refreshes
	seen map[string]time.Time // gateway file -> modification time last loaded
}

// UseServerGateways merges the gateway files bbaccess wrote into dir with the
// zones from the config file, and makes Refresh follow them. A config-file zone
// replaces a server-sent zone with the same suffix, which is how a gateway is
// pointed somewhere else for testing; any other overlap is an error. Calling
// it again, for a different dir, starts over from the config-file zones.
func (c *Config) UseServerGateways(dir string) error {
	user := c.Zones
	if c.server != nil {
		user = c.server.user
	}
	c.server = &serverZones{dir: dir, user: user}
...
func (c *Config) Refresh() (changed bool, err error) {
	...
	for _, f := range slices.Sorted(maps.Keys(now)) {
		...
		for _, z := range g.zones(strings.TrimSuffix(base, GatewaysSuffix), base) {
			if overridden(s.user, z) {
				continue
			}
...
func overridden(user []Zone, z Zone) bool {
	for _, u := range user {
		if normalize(u.Suffix) == normalize(z.Suffix) {
			return true
		}
	}
	return false
