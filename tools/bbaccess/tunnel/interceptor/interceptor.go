...
func New(dev tun.Device, cfg *tunnelconfig.Config, table *fakeip.Table, dialer Dialer) (*Interceptor, error) {
	mtu, err := dev.MTU()
	if err != nil {
...
func (i *Interceptor) connect(r *tcp.ForwarderRequest, zone tunnelconfig.Zone, name, target string, port int) {
	start := time.Now()
