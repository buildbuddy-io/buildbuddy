...
// fails quickly.
func unreachableGateway(t *testing.T) tunnelconfig.Zone {
	t.Helper()
	l, err := net.Listen("tcp", "127.0.0.1:0")
	require.NoError(t, err)
	addr := l.Addr().String()
	require.NoError(t, l.Close())
	return tunnelconfig.Zone{Suffix: "foo.bb.internal", Gateway: "grpc://" + addr}
}

// noCredentials attaches nothing: these tests never reach a gateway.
type noCredentials struct{}

func (noCredentials) Attach(ctx context.Context, _ tunnelconfig.Zone, _ string) (context.Context, error) {
	return ctx, nil
}
