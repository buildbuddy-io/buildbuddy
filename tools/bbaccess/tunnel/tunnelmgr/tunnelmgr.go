...
//
// Nothing here runs until traffic arrives: the first DNS query or TCP
// connection for a zone triggers registration + handshake, concurrent callers
// coalesce onto that one bring-up, and an idle tunnel is torn down. This is
// the "on demand" half of the design — the tunnel exists only while a
// developer is actually using it.
//
// Registration uses the streaming Connect RPC: the gateway leases the
// registration to the stream, so closing the stream is deregistration, and the
// gateway ending the stream is how the daemon learns it was evicted (gateway
// restart, credential expiry, stale-peer sweep) — it re-registers on the next
// use. The bring-up otherwise mirrors cli/ssh: generate an ephemeral keypair
// and configure a wireguard-go device backed by a gVisor netstack. No root, no
// kernel interface, no persistent key.
//
// Credentials are minted per registration rather than held for the daemon's
// lifetime, because an employee credential is bound to the WireGuard key being
// registered — see server/util/relayauth.
package tunnelmgr
...
// which happens on every gateway restart, since it generates a fresh server
// key at startup and reallocates addresses. This is a backstop: the Connect
// stream ending usually reports eviction well before the handshakes go quiet.
const handshakeStaleAfter = 210 * time.Second

// failureBackoff is how long a failed bring-up is remembered. Without it, an
// application retrying a name against a gateway that is down produces one
// registration attempt per DNS query, from every developer at once.
...
// server/util/relayauth. It is called once per registration rather than once
// per daemon, so an implementation can reload a refreshed certificate.
type Credentials interface {
	Attach(ctx context.Context, zone config.Zone, wgPublicKey string) (context.Context, error)
}
...
//
// The name is resolved by the gateway, not here: that is what makes a
// per-cluster gateway able to serve that cluster's names with no extra config.
func (m *Manager) Dial(ctx context.Context, zone config.Zone, host string, port int) (net.Conn, error) {
	...
	// is merely slow would kill the established sessions riding it — other
	// people's shells and port-forwards — to fix one failed dial.
...
func (m *Manager) Prewarm(ctx context.Context, zone config.Zone) {
...
// several peers.
func (m *Manager) get(ctx context.Context, zone config.Zone) (*tunnel, error) {
	m.mu.Lock()
