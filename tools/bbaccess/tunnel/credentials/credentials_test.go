...
func TestAudience(t *testing.T) {
	// Derived from the gateway host by default, so the credential is bound to
	// the host we believe we are talking to.
	got, err := Audience(config.Zone{Gateway: "grpcs://gateway.foo.example:443"})
	require.NoError(t, err)
	assert.Equal(t, "gateway.foo.example", got)

	// An explicit audience wins, for the cases where the target is an address.
	got, err = Audience(config.Zone{Gateway: "grpc://127.0.0.1:1985", Audience: "gateway.bar.example"})
	require.NoError(t, err)
	assert.Equal(t, "gateway.bar.example", got)

	_, err = Audience(config.Zone{Gateway: ""})
	assert.Error(t, err)
}
...
func TestStoreRoundTrip(t *testing.T) {
	...
	require.NoError(t, store.Write("certs_buildbuddy_io", certPEM, keyPEM))

	names, err := store.Names()
	require.NoError(t, err)
	assert.Equal(t, []string{"certs_buildbuddy_io"}, names)

	signer, err := store.Load("certs_buildbuddy_io")
	require.NoError(t, err)
...
func TestGatewaysRoundTrip(t *testing.T) {
	store, err := NewStore(t.TempDir())
	require.NoError(t, err)
	require.NoError(t, store.WriteGateways("certs_example", config.ServerGateways{Gateways: []config.ServerGateway{
		{Target: "grpcs://foo.bar.example", Zones: []config.ServerZone{{Suffix: "foo.bb.internal", RewriteTo: "cluster.local"}}},
	}}))
...
func TestStoreKeyIsNotWorldReadable(t *testing.T) {
	...
	require.NoError(t, err)
	certPEM, keyPEM := ca.IssuePEM(t, "someone@example.com", time.Time{})
	require.NoError(t, store.Write("prod", certPEM, keyPEM))
...
func TestLoadWithSeveralCredentialsAsksWhichOne(t *testing.T) {
	...
	store, err := NewStore(t.TempDir())
	require.NoError(t, err)
	for _, name := range []string{"certs_buildbuddy_io", "certs_buildbuddy_dev"} {
		certPEM, keyPEM := ca.IssuePEM(t, "someone@example.com", time.Time{})
		require.NoError(t, store.Write(name, certPEM, keyPEM))
	}
...
func TestAttachMintsACredentialForTheKey(t *testing.T) {
	...
	zone := config.Zone{Gateway: "grpcs://gateway.example", Credential: "prod"}
...
func TestAttachReloadsARefreshedCertificate(t *testing.T) {
	...
	require.NoError(t, store.Write("prod", stale, staleKey))
	...
	require.NoError(t, store.Write("prod", fresh, freshKey))
...
func TestAPIKeyAttach(t *testing.T) {
	ctx, err := APIKey("secret").Attach(context.Background(), config.Zone{}, "wgkey")
	require.NoError(t, err)
	md, ok := metadata.FromOutgoingContext(ctx)
	require.True(t, ok)
	assert.Equal(t, []string{"secret"}, md.Get("x-buildbuddy-api-key"))

	// An empty key attaches nothing rather than an empty header.
	ctx, err = APIKey("").Attach(context.Background(), config.Zone{}, "wgkey")
	require.NoError(t, err)
