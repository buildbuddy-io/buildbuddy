...
func TestFetchBlob_LocalSingleflight(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			flightRequestsBefore := testmetrics.CounterValue(t, metrics.RemoteAssetFetchSingleflightRequests)
			flightExecutionsBefore := testmetrics.CounterValue(t, metrics.RemoteAssetFetchSingleflightExecutions)
			t.Cleanup(func() {
				wantFlightRequests := tc.wantRequests
				if tc.wantShared {
					wantFlightRequests++
				}
				require.Equal(t, float64(wantFlightRequests), testmetrics.CounterValue(t, metrics.RemoteAssetFetchSingleflightRequests)-flightRequestsBefore)
				require.Equal(t, float64(tc.wantRequests), testmetrics.CounterValue(t, metrics.RemoteAssetFetchSingleflightExecutions)-flightExecutionsBefore)
			})
			...
			if tc.secondWantFailure {
				require.Equal(t, int32(gcodes.NotFound), got.response.GetStatus().GetCode())
				require.Contains(t, got.response.GetStatus().GetMessage(), "failed to upload")
...
func TestFetchBlob_RepeatedHeadersAndURIOverride(t *testing.T) {
	flags.Set(t, "storage.tempdir", t.TempDir())
	require.NoError(t, scratchspace.Init())
	ctx := context.Background()
	te := testenv.GetTestEnv(t)
	...
	first := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		firstRequests.Add(1)
		assert.Equal(t, []string{"shared-1", "shared-2"}, r.Header.Values("shared"))
		assert.Equal(t, []string{"first-2"}, r.Header.Values("override"))
		http.Error(w, "no blob here", http.StatusForbidden)
	}))
	defer first.Close()
	second := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, []string{"shared-1", "shared-2"}, r.Header.Values("shared"))
		assert.Equal(t, []string{"shared-override-1", "shared-override-2"}, r.Header.Values("override"))
		fmt.Fprint(w, "some blob")
	}))
	defer second.Close()
...
func TestFetchBlob_InvalidChecksum(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"empty_hash", "sha256-", ""},
		{"empty_hash_with_option", "sha256-?option", ""},
...
func TestFetchBlob_ChecksumSRIList(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:              "single_checksum_with_option",
			checksum:          sha256CRI + "?option",
			wantResponseCode:  gcodes.OK,
			wantOriginFetches: 1,
		},
		{
			name:              "alternative_checksums_with_options",
			checksum:          wrongSHA256CRI + "?option " + sha512CRI + "?other",
			chunked:           true,
			wantResponseCode:  gcodes.OK,
			wantOriginFetches: 1,
		},
	...
	for _, compressed := range []bool{false, true} {
		for _, chunked := range []bool{false, true} {
			t.Run(fmt.Sprintf("compression=%t/chunked=%t", compressed, chunked), func(t *testing.T) {
				...
				conn := runFetchServer(ctx, t, te)
				if !chunked {
					// A checksum-qualified known-length fetch must stream directly
					// to CAS, even when it is eligible for a shared flight.
					require.NoError(t, os.RemoveAll(filepath.Join(scratchRoot, "buildbuddy-scratch")))
				}
				origin := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					if chunked {
