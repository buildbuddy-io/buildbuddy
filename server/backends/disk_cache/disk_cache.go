...
func (p *partition) reduceCacheSize(targetSize int64) bool {
	...
	value, lastAccessNanos, ok := p.lru.RemoveOldest()
	if !ok {
		return false // should never happen
	}
	if fr, ok := value.(*fileRecord); ok {
		log.Debugf("Delete thread removed item from cache. Last use (nanos) was: %d", lastAccessNanos)
		p.liveRemove(fr)
	}
...
func (p *partition) getLRU(key *fileKey) (*fileRecordWrapper, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	v, lastAccessNanos, contains := p.lru.Get(key.FullPath())
	if !contains {
		if !p.diskIsMapped {
