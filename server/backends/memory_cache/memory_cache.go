...
func (m *MemoryCache) updateAtime(ctx context.Context, rn *rspb.ResourceName) error {
	for _, updater := range m.env.GetAtimeUpdaters() {
		updater.Enqueue(ctx, rn.GetInstanceName(), []*repb.Digest{rn.GetDigest()}, rn.GetDigestFunction())
	}
	return nil
}
