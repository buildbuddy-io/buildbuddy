...
func (r *statsRecorder) flushInvocationStatsToOLAPDB(ctx context.Context, ij *invocationInfo) error {
	...
	// clean up the reverse invocation links, since these are only needed for
	// listing in-progress executions linked to an invocation, and this listing
	// will now be queryable using ClickHouse.
	defer func() {
		if err := r.env.GetExecutionCollector().DeleteReverseInvocationLinks(ctx, inv.InvocationID); err != nil {
	...
	for {
		...
		if err != nil {
			log.CtxErrorf(ctx, "Failed to flush executions to clickhouse: %s", err)
			break
		}
		log.CtxInfof(ctx, "Successfully wrote %d executions for invocation", len(executions))
