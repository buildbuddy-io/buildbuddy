...
func (v *BEValues) handleStartedEvent(event *build_event_stream.BuildEvent) {
	...
	// The Started event is expected to pre-declare all of the build events that
	// affect invocation-level metadata. Add all of these metadata events to a
	// set keeping track of the events we're still expecting.
	for _, childEventID := range event.Children {
