...
func (s *ByteStreamServer) Read(req *bspb.ReadRequest, stream bspb.ByteStream_ReadServer) error {
	...
	downloadTracker.CloseWithBytesTransferred(bytesFromCache, int64(bytesTransferredToClient), r.GetCompressor(), "BSS::Read")
	return err
}
...
func (s *ByteStreamServer) Write(stream bspb.ByteStream_WriteServer) error {
	...
	for {
		...
		if streamState == nil { // First message
			...
			defer func() {
				uploadTracker.CloseWithBytesTransferred(streamState.offset, int64(bytesUploadedFromClient), streamState.resourceName.GetCompressor(), "BSS:Write")
			}()
...
func (s *ByteStreamServer) handleAlreadyExists(ctx context.Context, stream bspb.ByteStream_WriteServer, firstRequest *bspb.WriteRequest) error {
	...
	if r.GetCompressor() != repb.Compressor_IDENTITY {
		...
		if !firstRequest.FinishWrite {
			...
			uploadTracker.CloseWithBytesTransferred(0, int64(len(firstRequest.Data))+remainingSize, r.GetCompressor(), "BSS:handleAlreadyExists")
			if err != nil {
				return err
			}
		}
