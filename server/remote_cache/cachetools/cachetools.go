...
func BatchReadBlobs(ctx context.Context, casClient repb.ContentAddressableStorageClient, req *repb.BatchReadBlobsRequest) (map[string]*BlobResponse, error) {
	...
	for _, res := range res.Responses {
		...
		results[res.GetDigest().GetHash()] = &BlobResponse{Data: data}
	}
	if len(expected) > 0 {
...
// BatchReadSingleBlob reads a single blob using the BatchReadBlobs API. It is
// slightly more efficient than GetBlob as it avoids streaming overhead, but can
