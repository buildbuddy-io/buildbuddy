...
func uploadFromReader(ctx context.Context, bsClient bspb.ByteStreamClient, r *digest.CASResourceName, in io.Reader) (*repb.Digest, int64, error) {
	...
	if err != nil {
		// If there is a hash mismatch, re-hash to check whether a concurrent
...
// computed previously. Returns an error if the digests do not match.
func checkConcurrentMutation(originalDigest *repb.Digest, digestFunction repb.DigestFunction_Value, r io.ReadSeeker) error {
	if _, err := r.Seek(0, io.SeekStart); err != nil {
		return status.InternalErrorf("seek: %s", err)
	}
	computedDigest, err := digest.Compute(r, digestFunction)
	if err != nil {
		return status.InternalErrorf("recompute digest: %s", err)
	}
	if !digest.Equal(computedDigest, originalDigest) {
		return status.InternalErrorf("concurrent mutation detected: digest changed from %s to %s", digest.String(originalDigest), digest.String(computedDigest))
	}
	return nil
}
