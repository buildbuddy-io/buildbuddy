...
func TestUploadWriteCloser_InterleaveWritesReadFrom(t *testing.T) {
	...
	uwc, err := cachetools.NewUploadWriteCloser(ctx, te.GetByteStreamClient(), casrn)
	require.NoError(t, err)
	{
		n, err := uwc.Write(buf[:quarter])
		require.NoError(t, err)
		require.Equal(t, quarter, n)
	}
	{
		n, err := uwc.ReadFrom(bytes.NewReader(buf[quarter : 2*quarter]))
		require.NoError(t, err)
		require.Equal(t, int64(quarter), n)
	}
	{
		n, err := uwc.Write(buf[2*quarter : 3*quarter])
		require.NoError(t, err)
		require.Equal(t, quarter, n)
	}
	{
		n, err := uwc.ReadFrom(bytes.NewReader(buf[3*quarter:]))
		require.NoError(t, err)
		require.Equal(t, int64(quarter), n)
	}
	err = uwc.Close()
...
func TestUploadWriteCloser_NoWritesReadFromsAfterClose(t *testing.T) {
	...
	n, err := uwc.ReadFrom(bytes.NewReader(buf))
	require.Error(t, err)
