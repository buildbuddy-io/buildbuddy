...
// resultKey returns a redis key that stores a serialized ScoreCard Result
// for a particular result ID.
func resultKey(resultID string) string {
	return "hit_tracker/result/" + resultID
}
...
func (t *transferTimer) Close() error {
	// randDelay := rand.Intn(5)
	// time.Sleep(200*time.Microsecond*time.Duration(t.d.SizeBytes) + time.Duration(randDelay)*time.Millisecond) // DO NOT SUBMIT
...
func readResults(ctx context.Context, env environment.Env, iid string) *capb.ScoreCard {
	...
	resultIDs, err := c.SetMembers(ctx, resultIDsKey(iid))
	if err != nil {
		log.Warningf("Failed to read detailed scorecard: %s", err)
		return sc
	}
	sc.Results = make([]*capb.ScoreCard_Result, 0, len(resultIDs))
	for _, resultID := range resultIDs {
		// Read proto from redis
		// TODO: parallelize and/or pipeline this
		serializedResult, err := c.Get(ctx, resultKey(resultID))
		if err != nil {
