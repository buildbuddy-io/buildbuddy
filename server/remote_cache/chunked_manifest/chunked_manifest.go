...
func (cm *ChunkedManifest) Store(ctx context.Context, cache interfaces.Cache) error {
	...
	g.Go(func() error {
		...
		if len(missing) > 0 {
			return status.InvalidArgumentErrorf("required chunks not found in CAS: %+v", DigestsString(missing))
		}
		return nil
	})
	g.Go(func() error {
...
func DigestsString(digests []*repb.Digest) string {
	return strings.Join(digestsStrings(digests...), ", ")
}
