...
var chunkedManifestSalt = flag.String("cache.chunking.salt", "", "If set, salt the AC key with this value.")
...
func FromSplitBlobResponse(res *repb.SplitBlobResponse, blobDigest *repb.Digest, instanceName string, digestFunction repb.DigestFunction_Value) *ChunkedManifest {
	return &ChunkedManifest{
		BlobDigest:     blobDigest,
		ChunkDigests:   res.GetChunkDigests(),
...
func (cm *ChunkedManifest) Store(ctx context.Context, cache interfaces.Cache) error {
	...
	// These two validations are expensive. Background them to avoid blocking the main thread.
	//
	// TODO: This could be skipped if this manifest was previously verified, since AC is not shared between
	...
	g.Go(func() error {
		...
		for _, chunkDigest := range cm.ChunkDigests {
			rns = append(rns, digest.NewResourceName(chunkDigest, cm.InstanceName, rspb.CacheType_CAS, cm.DigestFunction).ToProto())
		}
		...
		if len(missing) > 0 {
			return status.NotFoundErrorf("manifest found with missing chunks %+v", missing)
		}
		return nil
	})
	g.Go(func() error {
...
func Load(ctx context.Context, cache interfaces.Cache, blobDigest *repb.Digest, instanceName string, digestFunction repb.DigestFunction_Value) (*ChunkedManifest, error) {
	...
	if err != nil {
		if status.IsNotFoundError(err) {
			return nil, status.NotFoundErrorf("chunked manifest not found for blob %s, %w", blobDigest.GetHash(), err)
		}
	...
	for _, outputFile := range ar.GetOutputFiles() {
		...
		rns = append(rns, digest.NewResourceName(outputDigest, instanceName, rspb.CacheType_CAS, digestFunction).ToProto())
	}

	// TODO: We could eventually return the missing chunks, and have the client
	...
	if len(missing) > 0 {
		return nil, status.NotFoundErrorf("manifest found with missing chunks %+v", missing)
	}
	return &ChunkedManifest{
		BlobDigest:     blobDigest,
