...
func rawName(actionCache bool, ct counterType) string {
	switch ct {
	case Hit:
...
func name(actionCache bool, ct counterType, iid string) string {
	return iid + "-" + rawName(actionCache, ct)
}
...
func (h *HitTracker) counterName(ct counterType) string {
	return name(h.actionCache, ct, h.iid)
}
...
func CollectCacheStats(ctx context.Context, env environment.Env, iid string) *capb.CacheStats {
	...
	cs.ActionCacheHits, _ = c.ReadCount(ctx, name(true, Hit, iid))
	cs.ActionCacheMisses, _ = c.ReadCount(ctx, name(true, Miss, iid))
	cs.ActionCacheUploads, _ = c.ReadCount(ctx, name(true, Upload, iid))

	cs.CasCacheHits, _ = c.ReadCount(ctx, name(false, Hit, iid))
	cs.CasCacheMisses, _ = c.ReadCount(ctx, name(false, Miss, iid))
	cs.CasCacheUploads, _ = c.ReadCount(ctx, name(false, Upload, iid))

	cs.TotalDownloadSizeBytes, _ = c.ReadCount(ctx, name(false, DownloadSizeBytes, iid))
	cs.TotalUploadSizeBytes, _ = c.ReadCount(ctx, name(false, UploadSizeBytes, iid))
	cs.TotalDownloadUsec, _ = c.ReadCount(ctx, name(false, DownloadUsec, iid))
	cs.TotalUploadUsec, _ = c.ReadCount(ctx, name(false, UploadUsec, iid))

	cs.DownloadThroughputBytesPerSecond, _ = c.ReadCount(ctx, name(false, DownloadThroughputBytesPerSecond, iid))
	cs.UploadThroughputBytesPerSecond, _ = c.ReadCount(ctx, name(false, UploadThroughputBytesPerSecond, iid))

	cs.TotalCachedActionExecUsec, _ = c.ReadCount(ctx, name(false, CachedActionExecUsec, iid))
