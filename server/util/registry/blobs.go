...
func toResourceName(h v1.Hash) *rspb.ResourceName {
	if h.Algorithm != "sha256" {
		panic("ONLY SEND SHA256 PLZ")
	}
	return &rspb.ResourceName{
		...
		// TODO(iain): because we don't know the size, we have to use AC for everything.
...
func (m *handler) Stat(ctx context.Context, _ string, h v1.Hash) (int64, error) {
	rn := toResourceName(h)
	if found, _ := m.cache.Contains(ctx, rn); !found {
		return 0, errNotFound
	}
	b, err := m.cache.Get(ctx, rn)
	if err != nil {
		return 0, errNotFound
	}
...
func (m *handler) Get(ctx context.Context, _ string, h v1.Hash) (io.ReadCloser, error) {
	rn := toResourceName(h)
	if found, _ := m.cache.Contains(ctx, rn); !found {
...
func (m *handler) Put(ctx context.Context, _ string, h v1.Hash, rc io.ReadCloser) error {
	defer rc.Close()
	all, err := io.ReadAll(rc)
	if err != nil {
		return err
	}
	return m.cache.Set(ctx, toResourceName(h), all)
}

func (m *handler) Delete(ctx context.Context, _ string, h v1.Hash) error {
	rn := toResourceName(h)
	if found, _ := m.cache.Contains(ctx, rn); !found {
		return errNotFound
	}
	return m.cache.Delete(ctx, rn)
}
...
func (b *blobs) handle(ctx context.Context, resp http.ResponseWriter, req *http.Request) *regError {
	elem := strings.Split(req.URL.Path, "/")
	elem = elem[1:]
	if elem[len(elem)-1] == "" {
		elem = elem[:len(elem)-1]
	...
	case http.MethodPost:
		...
		if digest != "" {
			...
			vrc, err := ReadCloser(req.Body, req.ContentLength, h)
			if err != nil {
				return regErrInternal(err)
			}
			defer vrc.Close()
	...
	case http.MethodDelete:
		...
		if err := bdh.Delete(ctx, repo, h); err != nil {
			return regErrInternal(err)
		}
		resp.WriteHeader(http.StatusAccepted)
		return nil
