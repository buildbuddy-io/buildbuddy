...
// SetObserver sets a prometheus observer to record compression ratio.
func (c *ZstdCompressingWriter) SetObserver(o prometheus.Observer) {
	c.observer = o
}
...
func NewZstdCompressingWriter(cwc interfaces.CommittedWriteCloser, bufferPool *bytebufferpool.VariableSizePool, bufSize int64) (*ZstdCompressingWriter, error) {
	if bufSize <= 0 {
