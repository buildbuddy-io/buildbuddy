...
var (
	...
	allowTrafficToHostDefaultIP   = flag.Bool("debug_allow_traffic_to_host_default_ip", false, "Allows traffic from VMs to the host's default IP address. For debugging/testing only", flag.Internal)
...
func setupVethPair(ctx context.Context, netns *Namespace) (_ *vethPair, err error) {
	...
	if *allowTrafficToHostDefaultIP {
		defaultIP, err := DefaultIP(ctx)
		if err != nil {
			return nil, err
		}
		iptablesRules = append(iptablesRules, []string{"FORWARD", "-i", vp.hostDevice, "-d", defaultIP.String(), "-j", "ACCEPT"})
		iptablesRules = append(iptablesRules, []string{"INPUT", "-i", vp.hostDevice, "-d", defaultIP.String(), "-j", "ACCEPT"})
	}
