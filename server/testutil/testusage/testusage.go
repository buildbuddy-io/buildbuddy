...
func (ut *Tracker) IncrementOLAP(ctx context.Context, labels map[sku.LabelName]sku.LabelValue, skuCounts map[sku.SKU]int64) error {
	c, err := claims.ClaimsFromContext(ctx)
	if err != nil {
		// Don't track unauth'd or anonymous requests.
		return nil
	}
	// Clone labels to avoid callers mutating the map after we record it.
	...
	for usageSKU, count := range skuCounts {
		...
		ut.olapIncrements = append(ut.olapIncrements, olapIncrement{c.GroupID, labelsCopy, usageSKU, count})
	}
	return nil
}
