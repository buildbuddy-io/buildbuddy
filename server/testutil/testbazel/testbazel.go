...
func BinaryPath(t testing.TB) string {
	...
	initOnce.Do(func() {
		...
		bazelrcLines := []string{
			...
			// rely on --vendor_dir (Bazel 9+) below instead, since
			// repository_cache entries can be platform-specific and may
			// not cover every host OS/arch.
		...
		if strings.HasPrefix(Version, "9.") {
			...
			// extract_bazel_installation via `bazel mod vendor` and is
			// platform-neutral (sources only, no binary artifacts).
			bazelrcLines = append(bazelrcLines, "common --vendor_dir="+vendorDirPath(t))
		}
