...
const APIKeyHeader = authutil.APIKeyHeader
...
func (a *TestAuthenticator) AuthContextFromAPIKey(ctx context.Context, apiKey string) context.Context {
	if apiKey == "" {
		log.Errorf("Failed to mint JWT (no API key provided)")
		return ctx
	}
	u := a.APIKeyProvider(apiKey)
	if u == nil {
		log.Errorf("API key %s is unknown to TestAuthenticator", apiKey)
		return ctx
	}
	jwt, err := u.AssembleJWT()
	if err != nil {
