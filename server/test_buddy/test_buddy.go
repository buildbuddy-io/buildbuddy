...
const (
	...
	defaultTargetPageSize      = 50
	maxTargetPageSize          = 200
	defaultAnalyzerRevision    = 1
	reportRequestTargetBytes   = 8_000_000
)
...
func Register(env *real_environment.RealEnv) error {
	...
	env.SetTestBuddyServiceClient(client)
...
func ReportTargetObservations(ctx context.Context, env environment.Env, repository string, observations []*tbpb.TestTargetObservation) error {
	client := env.GetTestBuddyServiceClient()
	if client == nil || len(observations) == 0 {
		return nil
	}
	stream, err := client.ReportTestResults(forwardAuth(ctx, env.GetAuthenticator()))
	if err != nil {
		return err
	}
	request := &tbpb.ReportTestResultsRequest{RepoUrl: repository}
	size := protowire.SizeTag(1) + protowire.SizeBytes(len(repository))
	flush := func() error {
		if len(request.GetTargetObservations()) == 0 {
			return nil
		}
	...
	for _, observation := range observations {
		wireSize := protowire.SizeTag(3) + protowire.SizeBytes(proto.Size(observation))
		if len(request.GetTargetObservations()) > 0 && size+wireSize > reportRequestTargetBytes {
			if err := flush(); err != nil {
				return err
			}
		}
		request.TargetObservations = append(request.TargetObservations, observation)
		size += wireSize
	}
	if err := flush(); err != nil {
		return err
	...
	if rsp.GetRejectedCount() != 0 {
		return status.InvalidArgumentErrorf("TestBuddy rejected %d target observations", rsp.GetRejectedCount())
	}
	return nil
}
...
func (p *proxy) GetTestTargets(ctx context.Context, req *tbpb.GetTestTargetsRequest) (*tbpb.GetTestTargetsResponse, error) {
	return p.client.GetTestTargets(forwardAuth(ctx, p.authenticator), req)
}
...
func (s *Service) reportTestResults(ctx context.Context, req *tbpb.ReportTestResultsRequest) (*tbpb.ReportTestResultsResponse, error) {
	...
	for _, observation := range report.CaseObservations {
		...
		if analyzer.Eligible(analyzer.Sample{Outcome: observationInfo.GetOutcome(), Source: observationInfo.GetSource()}) {
...
func observationProto(observation retainedObservation) *tbpb.TestObservation {
...
func analysisSamples(observations []retainedObservation) []analyzer.Sample {
	...
	for _, observation := range observations {
		out = append(out, analyzer.Sample{Outcome: observation.Outcome, Source: observation.Source})
	}
	return out
}
...
func (s *Service) GetTestTargets(ctx context.Context, req *tbpb.GetTestTargetsRequest) (*tbpb.GetTestTargetsResponse, error) {
	if req == nil {
		return nil, status.InvalidArgumentError("request is required")
	}
	groupID, err := s.groupID(ctx)
	...
	if page.GetLimit() > 0 {
		if pageSize > 0 && pageSize != page.GetLimit() {
			return nil, status.InvalidArgumentError("page_size cannot change while paging")
		}
	...
	if pageSize < 1 || pageSize > maxTargetPageSize {
		return nil, status.InvalidArgumentErrorf("page_size must be between 1 and %d", maxTargetPageSize)
	}
	if page.GetOffset() < 0 {
		return nil, status.InvalidArgumentError("page token contains a negative offset")
	}
	...
	filterClause := ""
	if len(req.GetHealthFilter()) > 0 {
		ranks := make([]any, 0, len(req.GetHealthFilter()))
		for _, value := range req.GetHealthFilter() {
			if _, ok := tbpb.TestHealth_name[int32(value)]; !ok {
				return nil, status.InvalidArgumentErrorf("unrecognized health filter %d", value)
			}
			switch value {
		...
		filterClause = `WHERE effective_health_rank IN (` + strings.TrimSuffix(strings.Repeat("?,", len(ranks)), ",") + `)`
		queryArgs = append(queryArgs, ranks...)
	}
	orderBy := "effective_health_rank ASC, effective_mean_duration_usec DESC, effective_pass_rate ASC, target_label ASC"
	direction := "ASC"
	if req.GetDescending() {
		direction = "DESC"
	}
	switch req.GetSort() {
	case tbpb.TestTargetSort_TEST_TARGET_SORT_HEALTH:
	case tbpb.TestTargetSort_TEST_TARGET_SORT_PASS_RATE:
		orderBy = "CASE WHEN effective_sample_count = 0 THEN 1 ELSE 0 END, effective_pass_rate " + direction +
			", effective_health_rank ASC, target_label ASC"
	case tbpb.TestTargetSort_TEST_TARGET_SORT_MEAN_DURATION:
		orderBy = "CASE WHEN effective_sample_count = 0 THEN 1 ELSE 0 END, effective_mean_duration_usec " + direction +
			", effective_health_rank ASC, target_label ASC"
	default:
		return nil, status.InvalidArgumentErrorf("unrecognized target sort %d", req.GetSort())
	}
	queryArgs = append(queryArgs, pageSize+1, page.GetOffset())
	query := fmt.Sprintf(`
			SELECT * FROM (
			SELECT target_health.*,
				CASE
					...
					ELSE 5 END AS effective_health_rank,
				CASE WHEN case_pass_count + case_fail_count + case_timeout_count > 0
					THEN case_pass_count * 1.0 / (case_pass_count + case_fail_count + case_timeout_count)
					ELSE COALESCE(pass_count * 1.0 / NULLIF(pass_count + fail_count + timeout_count, 0), 0)
					END AS effective_pass_rate,
				CASE WHEN case_pass_count + case_fail_count + case_timeout_count > 0
					THEN case_total_duration_usec / (case_pass_count + case_fail_count + case_timeout_count)
					ELSE COALESCE(total_duration_usec / NULLIF(pass_count + fail_count + timeout_count, 0), 0)
					END AS effective_mean_duration_usec,
				CASE WHEN case_pass_count + case_fail_count + case_timeout_count > 0
					THEN case_pass_count + case_fail_count + case_timeout_count
					ELSE pass_count + fail_count + timeout_count END AS effective_sample_count
			FROM (
		...
		LEFT JOIN "TestCaseStates" AS cs
			...
			%s
			ORDER BY %s
			LIMIT ? OFFSET ?`, where, filterClause, orderBy)
	type row struct {
		TargetLabel               string
		Disposition               int32
		Health                    string
	...
	rsp := &tbpb.GetTestTargetsResponse{Targets: make([]*tbpb.TestTargetSummary, 0, pageSize)}
	hasMore := false
	rq := s.env.GetDBHandle().NewQuery(ctx, "test_buddy_get_test_targets").Raw(query, queryArgs...)
	err = db.ScanEach(rq, func(ctx context.Context, r *row) error {
		if int64(len(rsp.Targets)) == pageSize {
			hasMore = true
			return nil
		}
		target, err := identity.CanonicalizeTarget(repository, r.TargetLabel)
	...
	if hasMore {
		next, err := paging.EncodeOffsetLimit(&pgpb.OffsetLimit{Offset: page.GetOffset() + pageSize, Limit: pageSize})
		if err != nil {
			return nil, err
		}
...
func (s *Service) GetTestTarget(ctx context.Context, req *tbpb.GetTestTargetRequest) (*tbpb.GetTestTargetResponse, error) {
	...
	for i := len(retained.Observations) - 1; i >= 0; i-- {
		rsp.RecentObservations = append(rsp.RecentObservations, observationProto(retained.Observations[i]))
	}
	caseFailures, err := s.recentCaseFailures(ctx, groupID, target)
	if err != nil {
		return nil, err
	}
	rsp.RecentCaseFailures = caseFailures
...
func (s *Service) recentCaseFailures(ctx context.Context, groupID string, target identity.TargetAddress) ([]*tbpb.TestCaseObservation, error) {
	type caseRow struct {
		CaseName           string
		RecentObservations []byte
	}
	var failures []*tbpb.TestCaseObservation
	rq := s.env.GetDBHandle().NewQuery(ctx, "test_buddy_get_target_case_failures").Raw(`
		SELECT cs.case_name, cs.recent_observations
		FROM "TestCaseStates" AS cs
		JOIN "TestCases" AS tc
			ON tc.group_id = cs.group_id AND tc.repository = cs.repository
			AND tc.target_label = cs.target_label AND tc.case_name = cs.case_name
		WHERE cs.group_id = ? AND cs.repository = ? AND cs.target_label = ?
			AND tc.deleted_at_usec = 0
			AND (cs.fail_count > 0 OR cs.timeout_count > 0)
		ORDER BY cs.case_name`, groupID, target.Repository, target.Label())
	if err := db.ScanEach(rq, func(ctx context.Context, row *caseRow) error {
		caseName, err := identity.CaseNameFromKey(row.CaseName)
		if err != nil {
			return err
		}
		...
		caseIdentity := identity.CaseProto(identity.CaseAddress{TargetAddress: target, CaseName: caseName})
		for i := len(retained.Observations) - 1; i >= 0; i-- {
			observation := retained.Observations[i]
			if observation.Outcome != tbpb.TestOutcome_TEST_OUTCOME_FAIL && observation.Outcome != tbpb.TestOutcome_TEST_OUTCOME_TIMEOUT {
				continue
			}
			failures = append(failures, &tbpb.TestCaseObservation{
				Identity: caseIdentity, Observation: observationProto(observation),
			})
		}
		return nil
	}); err != nil {
	...
	sort.SliceStable(failures, func(i, j int) bool {
		return failures[i].GetObservation().GetEventTimeUsec() > failures[j].GetObservation().GetEventTimeUsec()
	})
	return failures, nil
}
