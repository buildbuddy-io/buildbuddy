...
func getTestTargets(t *testing.T, service *testbuddy.Service, ctx context.Context, req *tbpb.GetTestTargetsRequest) []*tbpb.TestTargetSummary {
	t.Helper()
	pageRequest := proto.Clone(req).(*tbpb.GetTestTargetsRequest)
	var targets []*tbpb.TestTargetSummary
	for {
		response, err := service.GetTestTargets(ctx, pageRequest)
		require.NoError(t, err)
		targets = append(targets, response.GetTargets()...)
		if response.GetNextPageToken() == "" {
			return targets
		}
		pageRequest.PageToken = response.GetNextPageToken()
	}
}
...
func TestPresubmitObservationsDoNotAffectHealth(t *testing.T) {
	ctx := context.Background()
	env := testenv.GetTestEnv(t)
	service := testbuddy.New(env)
	repository := "https://github.com/acme/repo"
	...
	presubmitCase := caseObservation("presubmit-case", target, "TestCase", tbpb.TestOutcome_TEST_OUTCOME_FAIL, 10)
	presubmitCase.Observation.Source = tbpb.TestObservationSource_TEST_OBSERVATION_SOURCE_PRESUBMIT
	presubmitCase.Observation.FailureMessage = "presubmit failure"
	presubmitTarget := targetObservation("presubmit-target", target, tbpb.TestOutcome_TEST_OUTCOME_FAIL, 10)
	presubmitTarget.Observation.Source = tbpb.TestObservationSource_TEST_OBSERVATION_SOURCE_PRESUBMIT
	presubmitTarget.Observation.FailureMessage = "presubmit target failure"
	rsp, err := reportTestResults(service, ctx, &tbpb.ReportTestResultsRequest{
		RepoUrl: repository, CaseObservations: []*tbpb.TestCaseObservation{presubmitCase},
		TargetObservations: []*tbpb.TestTargetObservation{presubmitTarget},
	})
	require.NoError(t, err)
	require.Equal(t, int32(2), rsp.GetAcceptedCount())

	var count int64
	require.NoError(t, env.GetDBHandle().GORM(ctx, "test_buddy_test_count_case_catalog").
		Model(&tables.TestCase{}).Where("repository = ?", repository).Count(&count).Error)
	require.Equal(t, int64(1), count)
	require.NoError(t, env.GetDBHandle().GORM(ctx, "test_buddy_test_count_case_state").
		Model(&tables.TestCaseState{}).Where("repository = ?", repository).Count(&count).Error)
	require.Zero(t, count)
	require.NoError(t, env.GetDBHandle().GORM(ctx, "test_buddy_test_count_target_state").
		Model(&tables.TestTargetState{}).Where("repository = ?", repository).Count(&count).Error)
	require.Zero(t, count)
	postsubmit := caseObservation("postsubmit", target, "TestCase", tbpb.TestOutcome_TEST_OUTCOME_PASS, 20)
	postsubmit.Observation.Source = tbpb.TestObservationSource_TEST_OBSERVATION_SOURCE_POSTSUBMIT
	monitor := caseObservation("monitor", target, "TestCase", tbpb.TestOutcome_TEST_OUTCOME_PASS, 30)
	_, err = reportTestResults(service, ctx, &tbpb.ReportTestResultsRequest{
		RepoUrl: repository, CaseObservations: []*tbpb.TestCaseObservation{postsubmit, monitor},
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, tbpb.TestHealth_TEST_HEALTH_HEALTHY, detail.GetTest().GetSummary().GetHealth())
	require.Equal(t, int64(2), detail.GetTest().GetSummary().GetPassCount())
	require.Zero(t, detail.GetTest().GetSummary().GetFailCount())
	require.Len(t, detail.GetRecentObservations(), 2)
	require.ElementsMatch(t, []tbpb.TestObservationSource{
		tbpb.TestObservationSource_TEST_OBSERVATION_SOURCE_POSTSUBMIT,
		tbpb.TestObservationSource_TEST_OBSERVATION_SOURCE_MONITOR,
	}, []tbpb.TestObservationSource{
		detail.GetRecentObservations()[0].GetSource(),
		detail.GetRecentObservations()[1].GetSource(),
	})
}

func TestTargetDetailIncludesCaseFailures(t *testing.T) {
	ctx := context.Background()
	env := testenv.GetTestEnv(t)
	service := testbuddy.New(env)
	repository := "https://github.com/acme/repo"
	...
	first.Observation.FailureMessage = "first raw failure"
	second := caseObservation("case-2", target, "TestCase", tbpb.TestOutcome_TEST_OUTCOME_FAIL, 20)
	second.Observation.FailureMessage = "second raw failure"
	timeout := caseObservation("case-timeout", target, "OtherCase", tbpb.TestOutcome_TEST_OUTCOME_TIMEOUT, 40)
	targetFailure := targetObservation("target-1", target, tbpb.TestOutcome_TEST_OUTCOME_FAIL, 30)
	targetFailure.Observation.FailureMessage = "target raw failure"

	_, err := reportTestResults(service, ctx, &tbpb.ReportTestResultsRequest{
		RepoUrl: repository, CaseObservations: []*tbpb.TestCaseObservation{first, second, timeout},
	...
	require.Equal(t, "second raw failure", caseDetail.GetRecentObservations()[0].GetFailureMessage())
	require.Equal(t, "first raw failure", caseDetail.GetRecentObservations()[1].GetFailureMessage())
	...
	require.NoError(t, err)
	require.Len(t, targetDetail.GetRecentCaseFailures(), 3)
	outcomes := make(map[tbpb.TestOutcome]int)
	caseNames := make(map[string]int)
	for _, failure := range targetDetail.GetRecentCaseFailures() {
		outcomes[failure.GetObservation().GetOutcome()]++
		caseNames[failure.GetIdentity().GetCaseName()]++
	}
	require.Equal(t, 2, outcomes[tbpb.TestOutcome_TEST_OUTCOME_FAIL])
	require.Equal(t, 1, outcomes[tbpb.TestOutcome_TEST_OUTCOME_TIMEOUT])
	require.Equal(t, 2, caseNames["TestCase"])
	require.Equal(t, 1, caseNames["OtherCase"])
	require.Equal(t, "second raw failure", targetDetail.GetRecentCaseFailures()[1].GetObservation().GetFailureMessage())
	require.Equal(t, "first raw failure", targetDetail.GetRecentCaseFailures()[2].GetObservation().GetFailureMessage())
}
...
func TestGetRepositoryHealth(t *testing.T) {
	...
	require.Equal(t, int64(2), rsp.GetCases().GetHealthyCount())
	require.Equal(t, int64(1), rsp.GetCases().GetFlakyCount())
	require.Equal(t, int64(0), rsp.GetCases().GetInsufficientDataCount())
...
func TestConeOrdersHealthBySeverity(t *testing.T) {
	...
	report := func(target, name string, durationUsec int64, outcomes ...tbpb.TestOutcome) {
		for i, outcome := range outcomes {
			_, err := reportTestResults(service, ctx, &tbpb.ReportTestResultsRequest{
				RepoUrl: repository,
				CaseObservations: []*tbpb.TestCaseObservation{caseObservation(
					fmt.Sprintf("case-%s-%d", name, i), target, name, outcome, durationUsec)},
				TargetObservations: []*tbpb.TestTargetObservation{targetObservation(
					fmt.Sprintf("target-%s-%d", name, i), target, outcome, durationUsec)},
			})
			require.NoError(t, err)
		}
	}
	report("//pkg:failing", "TestFailing", 5_000, tbpb.TestOutcome_TEST_OUTCOME_FAIL)
	report("//pkg:flaky", "TestFlaky", 4_000,
	...
	for i, outcome := range []tbpb.TestOutcome{
		...
		_, err := reportTestResults(service, ctx, &tbpb.ReportTestResultsRequest{
			RepoUrl: repository,
			CaseObservations: []*tbpb.TestCaseObservation{caseObservation(
				fmt.Sprintf("case-only-%d", i), "//pkg:case_flaky", "TestCaseFlaky", outcome, 3_000)},
		})
		require.NoError(t, err)
	}
	report("//pkg:healthy", "TestHealthy", 2_000,
		tbpb.TestOutcome_TEST_OUTCOME_PASS,
		tbpb.TestOutcome_TEST_OUTCOME_PASS,
		tbpb.TestOutcome_TEST_OUTCOME_PASS)
	for i := 0; i < 5; i++ {
	...
	page1, err := service.GetTestTargets(ctx, &tbpb.GetTestTargetsRequest{RepoUrl: repository, PageSize: 2})
	require.NoError(t, err)
	require.Equal(t, []string{"//pkg:failing", "//pkg:flaky"}, targetLabels(page1.GetTargets()))
	require.NotEmpty(t, page1.GetNextPageToken())
	page2, err := service.GetTestTargets(ctx, &tbpb.GetTestTargetsRequest{
		RepoUrl: repository, PageSize: 2, PageToken: page1.GetNextPageToken(),
	})
	require.NoError(t, err)
	require.Equal(t, []string{"//pkg:case_flaky", "//pkg:timeout"}, targetLabels(page2.GetTargets()))
	require.NotEmpty(t, page2.GetNextPageToken())
	page3, err := service.GetTestTargets(ctx, &tbpb.GetTestTargetsRequest{
		RepoUrl: repository, PageSize: 2, PageToken: page2.GetNextPageToken(),
	})
	require.NoError(t, err)
	require.Equal(t, []string{"//pkg:healthy"}, targetLabels(page3.GetTargets()))
	require.Empty(t, page3.GetNextPageToken())

	flaky, err := service.GetTestTargets(ctx, &tbpb.GetTestTargetsRequest{
		RepoUrl: repository, HealthFilter: []tbpb.TestHealth{tbpb.TestHealth_TEST_HEALTH_FLAKY},
	})
	require.NoError(t, err)
	require.Equal(t, []string{"//pkg:flaky", "//pkg:case_flaky"}, targetLabels(flaky.GetTargets()))

	byDuration, err := service.GetTestTargets(ctx, &tbpb.GetTestTargetsRequest{
		RepoUrl: repository, Sort: tbpb.TestTargetSort_TEST_TARGET_SORT_MEAN_DURATION, Descending: true,
	})
	require.NoError(t, err)
	require.Equal(t, []string{
		"//pkg:failing", "//pkg:flaky", "//pkg:case_flaky", "//pkg:healthy", "//pkg:timeout",
	}, targetLabels(byDuration.GetTargets()))
}

func targetLabels(targets []*tbpb.TestTargetSummary) []string {
	labels := make([]string, 0, len(targets))
	for _, target := range targets {
		labels = append(labels, target.GetIdentity().GetTargetLabel())
	}
	return labels
}
...
func TestConeQueriesReturnAllResultsAcrossBatchesAndPages(t *testing.T) {
	ctx := context.Background()
	service := testbuddy.New(testenv.GetTestEnv(t))
	...
	targetRequest := &tbpb.GetTestTargetsRequest{RepoUrl: repository, PageSize: 200}
	listedTargets := make([]*tbpb.TestTargetSummary, 0, caseCount)
	targetPageCount := 0
	for {
		response, err := service.GetTestTargets(ctx, targetRequest)
		require.NoError(t, err)
		targetPageCount++
		...
		targetRequest.PageToken = response.GetNextPageToken()
	}
	require.Greater(t, targetPageCount, 1)
	...
	require.Equal(t, int64(0), rsp.GetCases().GetInsufficientDataCount())
	require.Equal(t, int64(caseCount)-failingCount, rsp.GetCases().GetHealthyCount())
