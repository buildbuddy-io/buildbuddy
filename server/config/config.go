...
type ExecutorConfig struct {
	...
	DockerSocketMountPath string `yaml:"docker_socket_mount_path" usage:"If set, mount the configured Docker socket to each action's container at this path, to enable Docker-on-Docker. Intended for backwards compatibility only; use Docker-in-Docker if possible."`
