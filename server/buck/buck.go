...
func ToBazelEvents(buildToolEvent *anypb.Any) ([]*bespb.BuildEvent, error) {
	var buckEvent buckdata.BuckEvent
	if err := buildToolEvent.UnmarshalTo(&buckEvent); err != nil {
		return nil, err
	}
	...
	case *buckdata.BuckEvent_SpanStart:
		...
		case *buckdata.SpanStartEvent_Command:
			...
			options := commandStart.GetCliArgs()
			var command string
		...
		case *buckdata.SpanStartEvent_ComputeDetailedAggregatedMetrics:
	...
	case *buckdata.BuckEvent_SpanEnd:
		...
		case *buckdata.SpanEndEvent_ComputeDetailedAggregatedMetrics:
	...
	case *buckdata.BuckEvent_Instant:
		...
		case *buckdata.InstantEvent_CellHasNewConfigs:
