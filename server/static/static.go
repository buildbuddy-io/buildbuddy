...
var (
	...
	usageAlertingRulesEnabled              = flag.Bool("app.usage_alerting_rules_enabled", false, "If set, show the usage alerting rules tab in the usage page.")
...
func serveIndexTemplate(ctx context.Context, env environment.Env, tpl *template.Template, version, jsPath, stylePath, appBundleHash string, w http.ResponseWriter) {
	config := cfgpb.FrontendConfig{
		...
		UsageAlertingRulesEnabled:              *usageAlertingRulesEnabled,
	}
