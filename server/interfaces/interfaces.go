...
type UsageTracker interface {
	...
	IncrementOLAP(ctx context.Context, labels map[sku.LabelName]sku.LabelValue, skuCounts map[sku.SKU]int64) error
}
...
type CommandResult struct {
	...
	// Stdout from the command. This may contain data even if there was an
	// Error. It is empty when stdout was redirected to a Stdio writer.
	Stdout []byte
	// Stderr from the command. This may contain data even if there was an
	// Error. It is empty when stderr was redirected to a Stdio writer.
	Stderr []byte
