...
It is called with two arguments, which are the paths to files
...
flags) and all the arguments that will be passed to the executable
generated by bazel (the arguments after the `--`), if any. **These
files each contain a single argument per line.**

The script will be called like this:

```bash
/usr/bin/env bash /path/to/plugin/pre_bazel.sh /path/to/bazel-args /path/to/executable-args
```
...
And here is an example of what the executable args file might look like (note that unlike bazel args, these cannot be canonicalized and thus will be simply split using standard shell lexing):
...
Your plugin can modify these files to add, remove, or change the flags that will ultimately be passed to Bazel or the resulting executable.
