...
snapshot matching strategy works optimally. This can be accomplished by triggering a Workflow on every push to the default branch, to ensure your default snapshot stays up to date. This ensures that Workflow runs on
new branches will start from an up-to-date base. Otherwise, if there is not an
optimal snapshot to resume from, workloads will be slower and incur higher startup costs.
...
On Linux, we recommend always running remote workloads on your default branch, so that the
