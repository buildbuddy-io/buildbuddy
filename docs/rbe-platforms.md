...
- `network`: controls the network access available in the VM/container.
  ...
  - `off`: The container/VM has no network access, not even to the host. This
    ...
    containers. The container/VM can still communicate with `localhost`.
  - `host`: The container/VM can communicate with itself and the host but
    nothing else. Only supported by `firecracker` VMs.
...
- `init-dockerd`: whether to start the `dockerd` process inside the VM
  ...
  `false`. Note: `host` or `external` network is required for `init-dockerd`.
